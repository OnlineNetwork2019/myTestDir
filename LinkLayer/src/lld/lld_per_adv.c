...
struct lld_per_adv_env_tag
{
    ...
    /// Offset of the Channel Map Update Indication
    uint8_t chm_up_ind_offset;
    ...
    uint8_t  big_act_id;
    /// Size of extended header in the exchange memory
    uint8_t  ext_hdr_em_len;
    /// Initialize group info
    bool     init_group_info;
    /// Update ACAD data
...
__STATIC void lld_per_adv_sched(uint8_t act_id)
{
    ...
    while ((!found) && (att < MAX_SCHED_ATT))
    {
        ...
        {
            // Increment priority
            evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_PER_ADV_IDX);
                                              ...
                                              uint16_t* data_len, bool force_aux_ptr)
{
    ...
    if (sync_ind)
    {
        ...
        #endif // (BLE_BIS)
        {
            res_ext_header_len += BLE_EXT_CHM_UPD_IND_LEN;
        }
    }
    ...
    if (sync_ind)
    {
        uint8_t chm_upd_ind[BLE_EXT_CHM_UPD_IND_LEN];

        // Save the Channel Map Update Indication offset
        adv_par->chm_up_ind_offset = offset;

        chm_upd_ind[0] = BLE_EXT_CHM_UPD_IND_LEN - 1; // Length of the data excluding this byte
        chm_upd_ind[1] = BLE_EXT_ACAD_CHANNEL_MAP_UPDATE_INDICATION_AD_TYPE;
        memcpy(&chm_upd_ind[2],&adv_par->chm.map[0], LE_CHNL_MAP_LEN);
        chm_upd_ind[7] = adv_par->chm_upd.instant;
        chm_upd_ind[8] = adv_par->chm_upd.instant >> 8;

        // Copy the Channel Map Update Indication
        em_wr((void *)&chm_upd_ind[0], *ext_header_txbuf_offset + offset, BLE_EXT_CHM_UPD_IND_LEN);

        offset += BLE_EXT_CHM_UPD_IND_LEN;
    }
    ...
    em_ble_txauxptr0_pack(txdesc_idx, /*txauxoffsetlsb*/ aux_offset, /*txauxoffsetunit*/ offset_units, /*txauxca*/ (lld_env.local_drift <= 50), /*txllch*/ adv_par->aux_ch_idx);
    ...
    if (sync_ind)
    {
        ...
        #if (BLE_BIS)
        // store information to properly transmit information of BIG
        if (big_info)
        {
            adv_par->init_group_info   = true;
            adv_par->ext_hdr_em_len    = offset - BLE_EXT_CHM_UPD_IND_LEN;
        }
        #endif // (BLE_BIS)
    }
...
__STATIC void lld_per_adv_evt_start_cbk(struct sch_arb_elt_tag* evt)
{
    ...
    if(evt != NULL)
    {
        ...
        {
            uint16_t ext_header_txbuf_offset = em_ble_txaedataptr_getf(sync_txdesc_idx);
            ...
            // Set the instant in the Channel Map Update Indication
            {
               ...
               uint8_t byte8 = adv_par->chm_upd.instant >> 8;
                ...
                // Copy 2 bytes
                em_wr((void *)&byte7, ext_header_txbuf_offset + adv_par->chm_up_ind_offset + 7, 1);
                em_wr((void *)&byte8, ext_header_txbuf_offset + adv_par->chm_up_ind_offset + 8, 1);
            }

            // Set the channel map in the Channel Map Update Indication
            em_wr((void *)&adv_par->chm_upd.map.map[0], ext_header_txbuf_offset + adv_par->chm_up_ind_offset + 2, LE_CHNL_MAP_LEN);

            // Update AUX_SYNC_IND Tx descriptor
            {
                uint8_t ext_header_len = adv_par->sync_ind_ext_hdr_len;

                if (ext_header_len == 0)
                {
                    ext_header_len = BLE_EXT_ADV_HEADER_FLAGS_LEN;
                }

                ext_header_len += BLE_EXT_CHM_UPD_IND_LEN;
        ...
        else if ((adv_par->chm_upd.new_chm_pending) && BLE_UTIL_INSTANT_PASSED(adv_par->chm_upd.instant, adv_par->pa_evt_cnt))
        {
           ...
           adv_par->chm_upd.new_chm_pending = false;
            ...
            // Update extended header length
            em_ble_txaeheader_txaelength_setf(sync_txdesc_idx, adv_par->sync_ind_ext_hdr_len);

            // Set advertising data Tx descriptor length to (extended header length + advertising data length + 1)
            em_ble_txphadv_txadvlen_setf(sync_txdesc_idx, (adv_par->sync_ind_ext_hdr_len + adv_par->sync_ind_adv_data_len + 1));
        }

        #if (BLE_BIS)
        if (!adv_par->chm_upd.new_chm_pending)
        {
            // check if some ACAD data should be added
            if(adv_par->big_act_id != BLE_INVALID_LINK_ID)
            {
                uint16_t em_ptr         = EM_BLE_ADVEXTHDRTXBUF_OFF(adv_par->act_id);
                uint8_t  ext_header_len = adv_par->sync_ind_ext_hdr_len;

                // request BIG activity to update ACAD data.
                uint8_t acad_len = lld_big_update_info(adv_par->big_act_id, evt->time.hs, &em_ptr,
                                                       adv_par->ext_hdr_em_len, adv_par->init_group_info);

                if(acad_len > 0)
                {
                    adv_par->init_group_info = false;
                    ...
                    ext_header_len += acad_len;
                }

                // update TX Descriptor
                em_ble_txaeheader_txaelength_setf(sync_txdesc_idx, ext_header_len);
                em_ble_txaedataptr_setf(sync_txdesc_idx, em_ptr);
                em_ble_txphadv_txadvlen_setf(sync_txdesc_idx, (ext_header_len + adv_par->sync_ind_adv_data_len + 1));
            }
            else
            {
...
uint8_t lld_per_adv_start(uint8_t act_id, struct lld_per_adv_params* params)
{
    ...
    if(lld_per_adv_env[act_id] == NULL)
    {
        ...
        if(lld_per_adv_env[act_id] != NULL)
        {
            ...
            // Extended advertiser
            em_ble_cntl_pack(cs_idx, RWIP_COEX_GET(ADV, TXBSY), RWIP_COEX_GET(ADV, RXBSY), RWIP_COEX_GET(ADV, DNABORT), EM_BLE_CS_FMT_EXT_ADV);
