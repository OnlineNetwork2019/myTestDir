...
struct lld_isoal_env_tag
{
    ...
    /// Trigger enable
    bool                        trigger_en;
    /// Trigger offset in microseconds from SDU Synchronization reference
...
__STATIC void lld_isoal_tx_get_unframed(struct lld_isoal_env_tag* p_env, uint8_t iso_buf_idx, uint32_t ref_anchor, bool rejected)
{
    ...
    if(p_sdu->remaining_pdu == 0)
    {
        ...
        if((p_env->p_dp != NULL) && (p_env->p_dp->cb_sdu_get != NULL))
        {
            ...
            p_sdu->p_buf = p_env->p_dp->cb_sdu_get(p_env->chan_hdl, &ref_anchor, &p_sdu->size);
        }
        else
        {
            p_sdu->p_buf   = NULL;
    ...
    else
    {
        ...
        if((pkt_len > 0) && (p_sdu->p_buf != NULL))
        {
            ...
            dma_copy(DMA_CHAN_ISOAL, (void*) em_buf_addr, &(p_sdu->p_buf[p_sdu->cursor]), pkt_len);
    ...
    if(p_sdu->remaining_pdu == 0)
    {
        ...
        if((p_env->p_dp != NULL) && (p_env->p_dp->cb_sdu_done != NULL))
        {
            ...
            p_env->p_dp->cb_sdu_done(p_env->chan_hdl, 0, 0, p_sdu->p_buf, p_sdu->status);
        }
...
__STATIC void lld_isoal_rx_done_unframed(struct lld_isoal_env_tag* p_env, uint8_t iso_buf_idx, uint32_t ref_anchor, bool rejected)
{
    ...
    if(p_sdu->remaining_pdu == 0)
    {
        ...
        if(p_env->next_sdu_sync_ref != 0)
        {
            ...
            uint16_t deviation = CO_MOD((p_env->next_sdu_sync_ref - (ref_anchor + p_env->sync_ref_offset)), p_env->sdu_interval);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // Use DMA to copy buffer
                dma_copy(DMA_CHAN_ISOAL, &(p_sdu->p_buf[p_sdu->cursor]), (void*) em_buf_addr, pkt_len);
            }
    ...
    if(p_sdu->remaining_pdu == 0)
    {
        ...
        if((p_env->p_dp != NULL) && (p_env->p_dp->cb_sdu_done != NULL))
        {
            ...
            p_env->p_dp->cb_sdu_done(p_env->chan_hdl, p_sdu->cursor, p_sdu->sdu_sync_ref, p_sdu->p_buf, p_sdu->status);
        }

        // If trigger is enabled
        if(p_env->trigger_en)
        {
            rwip_iso_timer_set(p_sdu->sdu_sync_ref + p_env->trigger_offset);
        }
    ...
    while(remain_len >= BLE_ISOAL_SEG_HEADER_SIZE)
    {
        ...
        if(p_sdu->cursor == p_sdu->size)
        {
            ...
            if((p_env->p_dp != NULL) && (p_env->p_dp->cb_sdu_get != NULL))
            {
                ...
                p_sdu->p_buf = p_env->p_dp->cb_sdu_get(p_env->chan_hdl, &sdu_ref_time, &p_sdu->size);
            }
            else
            {
                p_sdu->p_buf   = NULL;
        ...
        if(!rejected)
        {
            ...
            // Request DMA to copy SDU data segment
            dma_copy(DMA_CHAN_ISOAL, (uint8_t*)(EM_BASE_ADDR + em_buf_addr + pkt_cursor), p_sdu->p_buf + p_sdu->cursor, seg_len);
...
__STATIC void lld_isoal_rx_done_framed(struct lld_isoal_env_tag* p_env, uint8_t iso_buf_idx, uint32_t ref_anchor, bool rejected)
{
    ...
    if(rejected || (pkt_status == LLD_ISO_INVL_SYNC_ERR) || (pkt_status == LLD_ISO_INVL_CRC_ERR) || (pkt_llid != LLID_FRAMED_SEG))
    {
        ...
        if(p_sdu->p_buf != 0)
        {
            ...
            {
                // Indicate SDU reception is completed
                p_env->p_dp->cb_sdu_done(p_env->chan_hdl, p_sdu->size, p_sdu->sdu_sync_ref, p_sdu->p_buf, p_sdu->status);
            }
        ...
        if(p_env->next_sdu_sync_ref != 0)
        {
            ...
            if ( CLK_BTS_LOWER_EQ(p_env->next_sdu_sync_ref, (uint32_t) (ref_anchor + p_env->sync_ref_offset - p_env->sdu_interval)) )
            {
                ...
                {
                    // Indicate SDU lost
                    p_env->p_dp->cb_sdu_done(p_env->chan_hdl, 0, p_env->next_sdu_sync_ref, NULL, SDU_PKT_LOST);
                }

                // If trigger is enabled
                if(p_env->trigger_en)
                {
                    rwip_iso_timer_set(p_env->next_sdu_sync_ref + p_env->trigger_offset);
                }
        ...
        while((pkt_len - pkt_cursor) >= BLE_ISOAL_SEG_HEADER_SIZE)
        {
            ...
            if(GETB(seg_header, ISO_SEG_HDR_SC) == 0)
            {
                ...
                if(p_sdu->p_buf != NULL)
                {
                    ...
                    {
                        // Indicate SDU reception is completed
                        p_env->p_dp->cb_sdu_done(p_env->chan_hdl, p_sdu->size, p_sdu->sdu_sync_ref, p_sdu->p_buf, SDU_PARTIALLY_RECEIVED);
                    }
            ...
            else
            {
                ...
                if(p_sdu->p_buf != NULL)
                {
                    // Copy data into SDU buffer
                    dma_copy(DMA_CHAN_ISOAL, p_sdu->p_buf + p_sdu->cursor, (uint8_t*) (EM_BASE_ADDR + em_buf_addr + pkt_cursor), seg_len);
                }
            ...
            if(sdu_complete)
            {
                ...
                {
                    // Indicate SDU reception is completed
                    p_env->p_dp->cb_sdu_done(p_env->chan_hdl, p_sdu->size, p_sdu->sdu_sync_ref, p_sdu->p_buf, sdu_status);
                }
            ...
            if (sdu_status == SDU_UNEXPECTED_FORMAT)
                break;
        ...
        case RWIP_RST:
        {
            ...
            for(direction = 0 ; direction < ISO_SEL_MAX ; direction++)
            {
                for(idx = 0 ; idx < BLE_ACTIVITY_MAX ; idx++)
                {
                    ...
                    {
                        // If trigger is enabled
                        if(lld_isoal_env[direction][idx]->trigger_en)
                        {
                            rwip_iso_timer_clear();
                        }
        ...
        uint8_t bn, uint8_t max_pdu, uint8_t framing, bool mic_present, bool am0)
{
    ...
    p_env = (struct lld_isoal_env_tag*) ke_malloc(env_size, KE_MEM_ENV);
    ...
    p_env->max_sdu_deviation = sdu_interval * (BLE_MAX_DRIFT_SLEEP + lld_env.local_drift) / 1000000;
    p_env->max_sdu         = max_sdu;
    p_env->max_pdu         = max_pdu;
    p_env->chan_hdl        = chan_hdl;
    p_env->mic_present     = mic_present;
    p_env->framing         = framing;
    p_env->trigger_en      = false;
    p_env->am0             = am0;
...
void lld_isoal_stop(uint8_t act_id, uint8_t reason)
{
    ...
    for(direction = 0 ; direction < ISO_SEL_MAX ; direction++)
    {
        ...
        if(p_env != NULL)
        {
            ...
            {
                p_env->p_dp->cb_stop(p_env->chan_hdl, reason);
            }
...
uint8_t lld_isoal_datapath_set(uint8_t act_id, uint8_t direction, const struct data_path_itf* p_dp, bool trigger_enable, int32_t trigger_offset)
{
    ...
    else
    {
        ...
        {
            // Start data path
            status = p_dp->cb_start(p_env->chan_hdl, p_env->sdu_interval, p_env->trans_latency, p_env->max_sdu);
...
uint8_t lld_isoal_datapath_remove(uint8_t act_id, uint8_t direction)
{
    ...
    if(p_env != NULL)
    {
        if(p_env->p_dp != NULL)
        {
            ...
            {
                // Stop data path
                p_env->p_dp->cb_stop(p_env->chan_hdl, CO_ERROR_OPERATION_CANCELED_BY_HOST);
...
uint8_t lld_isoal_trigger_set(uint8_t act_id, uint8_t direction, bool enable, int32_t trigger_offset)
{
    uint8_t status = CO_ERROR_NO_ERROR;
    struct lld_isoal_env_tag* p_env;
    ASSERT_ERR(direction == ISO_SEL_RX);
    ASSERT_ERR(act_id < BLE_ACTIVITY_MAX);
    ...
    if(p_env != NULL)
    {
        ...
        {
            // Configure trigger
            p_env->trigger_offset = trigger_offset;
        }
        else
        {
            // Stop trigger timer if programmed
            rwip_iso_timer_clear(0);
        }

        // Store enable state
        p_env->trigger_en = enable;
    }
    else
    {
        status = CO_ERROR_UNKNOWN_CONNECTION_ID;
