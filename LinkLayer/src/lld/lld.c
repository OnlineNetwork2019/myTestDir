...
__STATIC void lld_rpa_renew_evt_canceled_cbk(struct sch_arb_elt_tag* evt)
{
    ...
    {
        // Increment priority
        evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_RPA_RENEW_IDX);
...
void lld_rxdesc_free(void)
{
    ...
    if (em_ble_rxdataptr_getf(rxdesc_idx) == 0)
    {
        ...
        if(em_ptr == 0)
        {
            // mark which rx_descriptor isn't ready for a proper data reception
...
void lld_channel_assess(uint8_t channel, bool rx_ok, uint32_t timestamp)
{
    if(channel < DATA_CHANNEL_NB)
    {
        ...
        if(rx_ok)
        {
            if(lld_env.ch_assess.level[channel] < BLE_CH_ASSESS_COUNT_MAX)
            {
                lld_env.ch_assess.level[channel]++;
            }
        }
        else
        {
            if(lld_env.ch_assess.level[channel] > BLE_CH_ASSESS_COUNT_MIN)
            {
                lld_env.ch_assess.level[channel]--;
            }
        }
        ...
        // Store timestamp
        lld_env.ch_assess.timestamp[channel] = timestamp;
    }
    else
    {
        ASSERT_ERR(0);
...
void lld_init(uint8_t init_type)
{
    switch (init_type)
    {
        ...
        case RWIP_RST:
        {
            // Reset/initialize RPA renew environment
            if(init_type && (lld_rpa_renew_env != NULL))
            {
                ke_free(lld_rpa_renew_env);
            }
        }
        ...
        case RWIP_1ST_RST:
        {
            ...
            // Initialize the maximum value of the local clock drift
            length = PARAM_LEN_LPCLK_DRIFT;
            if (rwip_param.get(PARAM_ID_LPCLK_DRIFT, &length, (uint8_t *) &lld_env.local_drift) != PARAM_OK)
            {
                // If no value is set in persistent storage, set the drift to 500 ppm (max value from BT spec)
                lld_env.local_drift = BLE_MAX_DRIFT_SLEEP;
            }

            // Deduce & save the SCA from the drift
            if (lld_env.local_drift < 21)
                lld_env.local_sca = SCA_20PPM;
            else if (lld_env.local_drift < 31)
                lld_env.local_sca = SCA_30PPM;
            else if (lld_env.local_drift < 51)
                lld_env.local_sca = SCA_50PPM;
            else if (lld_env.local_drift < 76)
                lld_env.local_sca = SCA_75PPM;
            else if (lld_env.local_drift < 101)
                lld_env.local_sca = SCA_100PPM;
            else if (lld_env.local_drift < 151)
                lld_env.local_sca = SCA_150PPM;
            else if (lld_env.local_drift < 251)
                lld_env.local_sca = SCA_250PPM;
            else
                lld_env.local_sca = SCA_500PPM;

            // Initialize the maximum value of the local active clock drift
            length = PARAM_LEN_ACTCLK_DRIFT;
            if (rwip_param.get(PARAM_ID_ACTCLK_DRIFT, &length, (uint8_t *) &lld_env.local_drift_active) != PARAM_OK)
            {
                // If no value is set in persistent storage, set the drift to 50 ppm (max value from BT spec)
                lld_env.local_drift_active = BLE_MAX_DRIFT_ACTIVE;
            }
...
bool lld_rxdesc_buf_ready(uint16_t em_ptr)
{
    ...
    if(lld_env.rxdesc_data_ptr_bf != 0)
    {
        ...
        // link to the descriptor
        em_ble_rxdataptr_setf(rxdesc_idx, em_ptr);
        // mark that rx_descriptor is ready for a proper data reception
        lld_env.rxdesc_data_ptr_bf &= ~CO_BIT(rxdesc_idx);
...
void lld_res_list_add(uint8_t position, struct bd_addr const *bd_addr, struct irk const *peer_irk, struct irk const *local_irk, uint8_t bd_addr_type, bool connected, bool in_wl)
{
    ...
    em_ble_ral_info_pack(position, /*entryvalid*/ 1, /*connected*/ connected,
...
void lld_res_list_peer_update(uint8_t position, bool connected)
{
    ...
    em_ble_ral_info_connected_setf(position, connected);
}
...
bool lld_calc_aux_rx(struct lld_calc_aux_rx_out* aux_rx_out, uint8_t index_pkt, uint32_t aux_data)
{
    ...
    if (aux_offset)
    {
        ...
        uint32_t drift = rwip_active_check() ? lld_env.local_drift_active : lld_env.local_drift;
        drift =  AUX_DRIFT_PPM(drift, ca); // calculate drift (ppm)
        ...
        uint32_t sync_win_size_us = 2*(BLE_NORMAL_WIN_SIZE + AUX_OFFSET_UNIT_USECS(offset_units) + ((uint32_t)(AUX_OFFSET_USECS(aux_offset,offset_units) * drift)/1000000));

        // Calculate total bit offset
        int32_t bit_offset = fine_cnt_rxsync + (aux_offset_us*2) - sync_win_size_us - 2*lld_exp_sync_pos_tab[em_ble_rxchass_rate_getf(index_pkt)];
        ...
        // The start of the sync window Rx calculation is the same for all rates
        // But in Long Range, we will need 8 symbols duration at 125Kbps to capture the sync from the start of the calculated sync window.
        if (aux_phy == AUX_PHY_CODED)
        {
            aux_rx_out->sync_win_size_us += (2*BLE_NORMAL_WIN_SIZE)*7;
        }
...
#if (BLE_BROADCASTER)
void lld_ch_map_set(const struct le_chnl_map *ch_map)
{
    uint8_t idx = 0;
