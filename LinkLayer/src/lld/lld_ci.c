...
/// Compute the flush instant for a given inserted data packets for a given channel
...
/// Get CIS structure
#define LLD_CIS_GET(act_id)      ((struct lld_cis_env *)lld_iso_env.p_channels[act_id])
/// Get CIG structure
#define LLD_CIG_GET(grp_hdl)     ((struct lld_cig_env *)lld_iso_env.p_groups[grp_hdl])
...
enum lld_cis_info_fields
{
    ...
    /// Indicate that sub events for this channel can be skipped
    ...
    /// Indicate if channel is encrypted
...
/// +---------+------------+-----------+---------+--------+------+-------+------+---------+------------+------+
/// | PROG_EN | ALARM_PROG | SCHED_REJ |  ALARM  |  INIT  | SYNC | SLAVE | WAIT | NEW_EVT | INTERLACED | PROG |
/// +---------+------------+-----------+---------+--------+------+-------+------+---------+------------+------+
///
enum lld_cig_info_fields
{
    /// Indicate that CIG event is in progress, cannot add/remove a channel and
    /// cannot update scheduling information
    LLD_CIG_INFO_PROG_POS         = 0,
    LLD_CIG_INFO_PROG_BIT         = 0x0001,

    /// Indicate that channels of this CIG are interlaced
    LLD_CIG_INFO_INTERLACED_POS   = 1,
    LLD_CIG_INFO_INTERLACED_BIT   = 0x0002,

    /// Indicate that no subevent for the CIG event has been programmed yet.
    /// If this bit is still set to 1 when CIG event is done, means that the CIG event has been entirely skipped.
    LLD_CIG_INFO_NEW_EVT_POS      = 2,
    LLD_CIG_INFO_NEW_EVT_BIT      = 0x0004,

    /// Indicate that CIG event is waiting for programming
    LLD_CIG_INFO_WAIT_POS         = 3,
    LLD_CIG_INFO_WAIT_BIT         = 0x0008,

    /// Indicate if we are slave of CIG
...
/// +---------+-------------+----------+---------+
/// |   RFU  | SUB_EVT_IDX | CHAN_HDL | GRP_HDL |
/// +---------+-------------+----------+---------+
///
enum lld_ci_dummy_fields
{
    ...
    /// Channel Handle
    LLD_CI_DUMMY_CHAN_HDL_LSB          = 8,
    LLD_CI_DUMMY_CHAN_HDL_MASK         = 0x0000FF00,
...
struct lld_ci_data
{
    ...
    uint8_t                          iso_descs_idx[BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN];
    /// Array of used ISO buffers
    uint8_t                          iso_bufs_idx[BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN];
...
struct lld_ci_se
{
    ...
    uint16_t            bit_offset;
    /// Activity id of channel this subevent belongs to
    uint8_t             chan_act_id;
    /// CIS Subevent index
...
struct lld_cig_env
{
    ...
    /// Bit field providing activity ids for channels to be stopped upon next end of CIG event
    uint32_t                    stop_activity_ids;

    /// List of channels belonging to this CIG (sorted by channel offset values)
    ...
    /// Last used cumulative drift value from connection (in half microseconds)
    int32_t                     last_drift_acc;
    ...
    uint32_t                    bts_array[__ARRAY_EMPTY];
};
...
struct lld_cis_env
{
    ...
    /// offset (in half microseconds) - Time from CIG anchor to CIS anchor
    ...
    uint16_t                         hop_ptr[2];
    /// Channel information (@see enum lld_cis_info_fields)
    uint16_t                         cis_info;
    /// Group Handle
    uint8_t                          grp_hdl;
    /// Activity identifier
...
#endif //(BLE_PERIPHERAL)
__STATIC void lld_ci_sched(struct lld_cig_env *p_cig);
 ...
 * @param[in] indicate     Indicate LLI task that driver has been stopped for the channel
...
__STATIC void lld_cis_free(struct lld_cis_env *p_cis, bool indicate, bool reset)
{
    ...
    if (!reset)
    {
        ...
        for (cnt = 0; cnt < BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN; cnt++)
        {
    ...
    // Update LLD CIS environment
    lld_iso_env.p_channels[p_cis->act_id] = NULL;
...
__STATIC void lld_cig_free(struct lld_cig_env *p_cig)
{
    ...
    lld_iso_env.p_groups[p_cig->grp_hdl] = NULL;
...
/**
 ****************************************************************************************
 * @brief Insert a channel CIS structure in the list of channel for a given CIG.
 ...
 ****************************************************************************************
 */
__STATIC void lld_cis_insert_channel(struct lld_cis_env *p_cis, struct lld_cig_env *p_cig)
{
    // Get first channel element
    struct lld_cis_env *p_list_channel = (struct lld_cis_env *)co_list_pick(&p_cig->list_cis);
    ...
    else
    {
        ...
        {
            if (p_cis->chan_offset <= p_list_channel->chan_offset)
            {
                // Insert the channel
                co_list_insert_before(&p_cig->list_cis, &p_list_channel->hdr, &p_cis->hdr);
                break;
            }

            // Get next element
            p_list_channel = (struct lld_cis_env *)(p_list_channel->hdr.next);
        }
...
/**
 ****************************************************************************************
 * @brief Set scheduling information for all sub events within a CIG (interlaced case)
 *
        ...
        while (p_cis)
        {
            ...
            if (GETB(p_cis->cis_info, LLD_CIS_INFO_STARTED))
            {
                if (sub_evt_cnt < p_cis->hop_inf.nse)
                {
                    ...
                    p_se->chan_act_id     = p_cis->act_id;
                    p_se->offset          = p_cis->chan_offset + (sub_evt_cnt * p_cis->sub_interval);
    ...
    while (p_cis)
    {
        ...
        if (GETB(p_cis->cis_info, LLD_CIS_INFO_STARTED))
        {
            ...
            for (se_cnt = 0; se_cnt < p_cis->hop_inf.nse; se_cnt++)
            {
                ...
                p_se->offset          = p_cis->chan_offset + (se_cnt * p_cis->sub_interval);
                p_se->state           = 0;
                p_se->nb_packed       = 0;
...
__STATIC uint8_t lld_cig_compute_sched(struct lld_cig_env *p_cig)
{
    ...
    p_cig->p_se_infos = ke_malloc(sizeof(struct lld_ci_se) * p_cig->cig_nse, KE_MEM_ENV);
    ...
    if (p_cig->p_se_infos)
    {
        ...
        if (GETB(p_cig->cig_info, LLD_CIG_INFO_INTERLACED))
        {
        ...
        p_cis = LLD_CIS_GET(p_se->chan_act_id);
 ...
 *
 * @param[in] p_cig      Pointer to the CIG structure the channel belongs to
 * @param[in] p_cis      Pointer to the CIS structure
 *
...
__STATIC void lld_cis_remove(struct lld_cig_env *p_cig, struct lld_cis_env *p_cis)
{
    ...
    // Decrease number of channels for this CIG
    p_cig->nb_cis--;
                                           ...
                                           struct lld_con_info_for_cis *p_con_cis_info)
{
    ...
    struct lld_cig_env *p_cig = (struct lld_cig_env *)ke_malloc(  sizeof(struct lld_cig_env)
    ...
    if (p_cig)
    {
        ...
        p_cig->last_drift_acc    = p_con_cis_info->sync_drift_acc;
        ...
        SETB(p_cig->cig_info, LLD_CIG_INFO_NEW_EVT, 1);
        ...
        lld_iso_env.p_groups[p_cis_params->grp_hdl] = p_cig;
        lld_iso_env.grp_type[p_cis_params->grp_hdl] = LLD_ISO_MODE_CI;
    }
 ...
 * @param[in]  p_con_cis_info    CIS parameters linked with BLE connection
 * @param[in]  p_cig             CIG structure
 * @param[in]  activity_id       CIS activity id
...
__STATIC struct lld_cis_env *lld_cis_alloc(struct lld_cis_params* p_cis_params,
                                           struct lld_con_info_for_cis* p_con_cis_info,
                                           struct lld_cig_env *p_cig, uint8_t activity_id,
                                           uint32_t* p_anchor_ts, uint32_t* p_anchor_bit_off)
{
    ...
    uint8_t* p_env       = (uint8_t*)ke_malloc(CO_ALIGN4_HI(sizeof(struct lld_cis_env)) + tx_env_size + rx_env_size,
                                               KE_MEM_ENV);
    ...
    if (p_cis)
    {
        // Channel anchor timestamp - half slot part
        uint32_t cis_anchor_ts;
        // Channel anchor timestamp - half microseconds part
        ...
        p_cis->act_id            = activity_id;
        p_cis->grp_hdl           = p_cis_params->grp_hdl;
        p_cis->chan_hdl          = p_cis_params->chan_hdl;
        ...
        #endif //(BLE_PERIPHERAL) || (BLE_PWR_CTRL)
        p_cis->chan_offset       = US_TO_HUS(p_cis_params->cis_spacing);
        ...
        // Update ISO environment
        lld_iso_env.p_channels[activity_id] = p_cis;
        lld_iso_env.chan_type[activity_id] = LLD_ISO_MODE_CI;

        // Insert the channel in the list of channel for the provided CIG
        lld_cis_insert_channel(p_cis, p_cig);

        // Increase number of CIS in the CIG
        p_cig->nb_cis++;

        // From second CIS, we cannot know if CISs are interlaced or not
        if (p_cig->nb_cis == 2)
        {
            SETB(p_cig->cig_info, LLD_CIG_INFO_INTERLACED, (p_cis_params->packing == ISO_PACKING_INTERLEAVED));
        }

        // Compute first anchor timestamp for the channel
        cis_anchor_ts      = p_con_cis_info->ref_timestamp;
        cis_anchor_bit_off = p_con_cis_info->ref_bit_off;

        // Update anchor position with provided connection offset (connection offset already includes the channel offset)
        ...
        // Check if we moved on next slot
        ...
        // Check if we moved on previous slot
        ...
        // Start point might be in the past, check how many channel events have been missed - Use a margin
        if(clock_diff > 0)
        {
        ...
        *p_anchor_bit_off   = cis_anchor_bit_off;
        ...
        // If first channel scheduled for the CIG, keep anchor position for the CIG
        if (p_cig->nb_cis == 1)
        {
...
__STATIC void lld_ci_compute_hop_scheme(struct lld_cis_env *p_cis, uint16_t hop_em_ptr, uint16_t evt_cnt)
{
    ...
    lld_iso_compute_hop_scheme(p_hop_inf);
}

/**
 ****************************************************************************************
 * @brief Prepare data transmission for the next channel event. It is considered here that
 ...
 * @param[in] start Use to know if we prepare data for first CIS event, else event is rescheduled
 ****************************************************************************************
 */
__STATIC void lld_cis_data_prepare(struct lld_cig_env *p_cig, struct lld_cis_env *p_cis, bool start)
{
    // Counter
    uint8_t cnt;
    struct lld_ci_data* p_tx = p_cis->p_tx;
...
/**
 ****************************************************************************************
 * @brief Program the TX ISO descriptors to be used during the next channel events.
...
__STATIC void lld_cis_data_tx_prog(struct lld_cig_env *p_cig, struct lld_cis_env *p_cis)
{
    ...
    if (p_tx->max_pdu)
    {
        uint8_t data_prog_idx = p_tx->data_idx;
        CO_VAL_ADD(data_prog_idx, p_tx->data_prog_nb, p_tx->data_queue_size);

        while (p_tx->data_prog_nb < p_tx->data_prep_nb)
        {
            ...
            lld_isoal_tx_get(p_cis->act_id, tx_buf_idx, p_cig->bts_array[bts_idx], false);

            // Update index of next TX ISO descriptor to be used
            CO_VAL_INC(p_tx->next_iso_idx, BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN);
...
/**
 ****************************************************************************************
 * @brief Program the RX ISO descriptors to be used during the next channel events.
...
__STATIC void lld_cis_data_rx_prog(struct lld_cis_env *p_cis)
{
    ...
    if (p_rx->max_pdu)
    {
        uint8_t data_prog_idx = p_rx->data_idx + p_rx->data_prog_nb;
        if(data_prog_idx >= p_rx->data_queue_size)
        {
            data_prog_idx -= p_rx->data_queue_size;
        }

        while (p_rx->data_prog_nb < p_rx->data_prep_nb)
        {
            // Get next data structure to be programmed
            ...
            // Check if descriptor can be used
            if (!em_ble_rxisoptr_rxdone_getf(rx_desc_idx))
            {
            ...
            CO_VAL_INC(p_rx->next_iso_idx, BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN);
    ...
    if (p_tx->max_pdu)
    {
        ...
        if (p_tx->data_prep_nb > 0)
        {
            ...
            if (p_data->flush_instant == p_cis->flush_cnt)
            {
                ...
                // Update index of next TX ISO descriptor to be used
                CO_VAL_INC(p_tx->curr_iso_idx, BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN);
                ...
                else
                {
                    ...
                    lld_isoal_tx_get(p_cis->act_id, BLE_UTIL_ISO_INDEX_INVALID, p_cig->bts_array[bts_idx], true);
                }
            }
        }
    ...
    if (p_rx->max_pdu)
    {
        ...
        if (p_rx->data_prep_nb > 0)
        {
            ...
            if (p_data->flush_instant == p_cis->flush_cnt)
            {
                ...
                // Update index of next RX ISO descriptor to be used
                CO_VAL_INC(p_rx->curr_iso_idx, BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN);
                // decrement number of element prepared
                p_rx->data_prep_nb--;
                ...
                lld_isoal_rx_done(p_cis->act_id, p_data->buf_idx, p_cig->bts_array[bts_idx], !p_data->prog);
...
__STATIC void lld_ci_alarm_cbk(struct sch_alarm_tag *p_alarm)
{
    ...
    if(GETB(p_cig->cig_info, LLD_CIG_ALARM_PROG))
    {
        ...
        struct lld_cis_env* p_cis = LLD_CIS_GET(p_se->chan_act_id);
        ...
        if(!cig_stop)
        {
            ...
            if(p_cig->prog_se_idx < (p_cig->ref_se_idx + p_ref_se->nb_packed))
            {
                ...
                if(GETB(p_cig->cig_info, LLD_CIG_INFO_SLAVE) && GETB(p_cig->cig_info, LLD_CIG_INFO_SYNC))
                {
                    ...
                    bit_off = p_next_se->exp_sync_bit_off + p_cig->slave_offset - BLE_NORMAL_WIN_SIZE/2;
    ...
    else if(GETB(p_cig->cig_info, LLD_CIG_SCHED_REJECT))
    {
        ...
        for (cnt = p_cig->in_proc_se_idx; cnt < max_se_idx; cnt++)
        {
            ...
            uint8_t chan_act_id = p_cig->p_se_infos[cnt].chan_act_id;

            // Handle end of sub event
            lld_ci_end_subevt(LLD_CIS_GET(chan_act_id), p_cig, true);
        }
    }
    else if (GETB(p_cig->cig_info, LLD_CIG_INFO_INIT))
    {
        ...
        p_cig->bts_array[p_cig->bts_idx] = rwip_bt_time_to_bts(p_cig->anchor_ts, p_cig->anchor_bit_off);
...
/**
 ****************************************************************************************
 * @brief Compute sync window for a slave connection event
 *
 * @param[in]  last_sync_ts      Time of the last synchronization to master (in half-slots)
 * @param[in]  target_ts         Time of the scheduling target (in half-slots)
 * @param[in]  master_sca        Master's sleep clock accuracy (in ppm)
 *
 * @return Synchronization window duration, in half-us
 ****************************************************************************************
 */
__STATIC uint32_t lld_ci_sync_win_compute(uint32_t last_sync_ts, uint32_t target_ts, uint16_t master_sca)
{
    ...
    uint32_t clock_diff_hs = CLK_SUB(target_ts, last_sync_ts);
    ...
    rx_win_size = co_max(rx_win_size, US_TO_HUS(BLE_NORMAL_WIN_SIZE));
...
__STATIC void lld_ci_sched(struct lld_cig_env *p_cig)
{
    ...
    do
    {
        ...
        if (GETB(p_cig->cig_info, LLD_CIG_INFO_SLAVE))
        {
            // retrieve first CIS of the CIG
            struct lld_cis_env* p_first_cis = (struct lld_cis_env*) co_list_pick(&(p_cig->list_cis));
            // Window widening limit, after which a disconnection occurs
            uint32_t win_lim = (p_first_cis->hop_inf.nse < 3) ? (p_cig->iso_interval*HALF_SLOT_SIZE/2 - US_TO_HUS(BLE_IFS_DUR)) : p_first_cis->sub_interval;

            // Compute synchronization window
            uint32_t rx_win_size = lld_ci_sync_win_compute(p_cig->last_sync_ts, target_clock, p_cig->master_sca);

            // If the window widening value has reached its limit, disconnect CIS due to synchronization timeout
            if((rx_win_size/2) >= win_lim)
            {
                timeout = true;
                break;
            }
            ...
            {
                // Retrieve half window size
                uint32_t half_win_size = rx_win_size >> 1;

                // Offset in half-slots
                uint32_t offset = (half_win_size) / HALF_SLOT_SIZE;

                // Compute the new window start position, RX windows centered on expected anchor point
                target_clock = CLK_SUB(target_clock, offset);
                bit_off -= (half_win_size - (offset * HALF_SLOT_SIZE));
        ...
        // Event has not been inserted, not waiting for programming
        SETB(p_cig->cig_info, LLD_CIG_INFO_WAIT, 0);

        // Event cannot be scheduled, increment priority
        p_evt->current_prio += RWIP_PRIO_INC(p_cig->prio_idx);
...
__STATIC void lld_cis_prog(struct lld_cig_env *p_cig, uint8_t current_prio)
{
    ...
    SETF(prog_par.dummy, LLD_CI_DUMMY_CHAN_HDL,    p_cis->act_id);
    ...
    if (GETB(p_cig->cig_info, LLD_CIG_INFO_SLAVE))
    {
        ...
        uint32_t sync_win_size_us = GETB(p_cig->cig_info, LLD_CIG_PROG_SYNC) ? BLE_NORMAL_WIN_SIZE : HUS_TO_US(p_cig->rx_win_size + 1);
...
__STATIC bool lld_cig_end_evt(struct lld_cig_env *p_cig)
{
    ...
    bool was_prog = GETB(p_cig->cig_info, LLD_CIG_INFO_PROG);

    // Update CIG anchor point parameters
    p_cig->anchor_ts = CLK_ADD_2(p_cig->anchor_ts, p_cig->iso_interval);
    p_cig->cig_evt_cnt++;

    // CIG event was never programmed so fully rejected
    if (!was_prog)
    {
        // Mark that new CIG event start is over
        SETB(p_cig->cig_info, LLD_CIG_INFO_NEW_EVT, 0);
    }
    ...
    if (cig_continue)
    {
        ...
        while (p_cis)
        {
            ...
            if (GETB(p_cis->cis_info, LLD_CIS_INFO_STARTED))
            {
                ...
                lld_cis_data_prepare(p_cig, p_cis, false);

                if (!was_prog)
                {
                    // Restart computation of hopping for next event
                    lld_ci_compute_hop_scheme(p_cis, p_cis->hop_ptr[GETB(p_cis->cis_info, LLD_CIS_INFO_HOP_TOG)],
                                              p_cis->cis_evt_cnt + p_cis->evt_inc);
                }
            }
            else
...
__STATIC bool lld_ci_end_sched_act(struct lld_ci_se *p_ref_se, struct lld_cig_env *p_cig)
{
        ...
        GETB(p_cig->cig_info, LLD_CIG_INFO_SYNC))
    {
        ...
        p_cig->last_drift_acc += p_cig->slave_offset;
        ...
        lld_con_sync_time_update(p_cig->con_link_id, p_cig->last_sync_ts, p_cig->last_drift_acc);
    }
    #endif //(BLE_PERIPHERAL)
    ...
    while (p_cig->ref_se_idx < p_cig->cig_nse)
    {
        ...
        for (cnt = p_cig->ref_se_idx; cnt < (p_cig->ref_se_idx + p_ref_se->nb_packed); cnt++)
        {
            ...
            uint8_t chan_act_id = p_cig->p_se_infos[cnt].chan_act_id;
            // Get CIS structure
            struct lld_cis_env *p_cis = LLD_CIS_GET(chan_act_id);
...
__STATIC bool lld_ci_end_subevt(struct lld_cis_env *p_cis, struct lld_cig_env *p_cig, bool rejected)
{
    ...
    if (rejected)
    {
        ...
        uint8_t cs_idx = EM_BLE_CS_ACT_ID_TO_INDEX(p_cur_se->chan_act_id);
    ...
    if (!GETB(p_cis->cis_info, LLD_CIS_INFO_SKIP))
    {
        // Check if all sub-events for this channel have been processed
        if (p_cis->sub_evt_cnt != 0)
        {
            struct lld_ci_data* p_tx = p_cis->p_tx;
            struct lld_ci_data* p_rx = p_cis->p_rx;

            // Check if next sub-events for this channel need to be scheduled, if it has not already
...
/**
 ****************************************************************************************
 * @brief Process RSSI information
 ****************************************************************************************
 */
__STATIC void lld_ci_rssi_update(struct lld_cis_env *p_cis, bool rx_ok)
{
    ...
    if (lld_rxdesc_check(EM_BLE_CS_ACT_ID_TO_INDEX(p_cis->act_id)))
    {
        ...
        do
        {
            ...
            if (GETB(p_cig->cig_info, LLD_CIG_INFO_SLAVE))
            {
                ...
                // Adjust if we are now on previous slot
                while (new_bit_off < 0)
                {
            ...
            {
                // Skip next subevents for this channel
            ...
            if (rxstatcis & EM_BLE_MIC_ERR_BIT)
            {
                ...
                // Keep also the activity id for the channel to be stopped
                p_cig->stop_activity_ids |= CO_BIT(p_cis->act_id);
        ...
        lld_ci_rssi_update(p_cis, rx_ok);
...
__STATIC void lld_ci_evt_start_cbk(struct sch_arb_elt_tag *p_evt)
{
    ...
    if (p_evt != NULL)
    {
        ...
        // New event of a CIG
        if (GETB(p_cig->cig_info, LLD_CIG_INFO_NEW_EVT))
        {
            // Get first channel information
            struct lld_cis_env *p_cis = (struct lld_cis_env *)co_list_pick(&p_cig->list_cis);
            ...
            while (p_cis)
            {
                // Ensure that channel is started
                if (GETB(p_cis->cis_info, LLD_CIS_INFO_STARTED))
                {
                    uint8_t hop_tog = GETB(p_cis->cis_info, LLD_CIS_INFO_HOP_TOG);

                    // Update hopping sequence to use
                    em_ble_hopptr_hop_seq_ptr_setf(EM_BLE_CS_ACT_ID_TO_INDEX(p_cis->act_id),
                                                   p_cis->hop_ptr[hop_tog] >> 2);

                    // Toggle Hop TOG bit
                    hop_tog = !hop_tog;

                    // Request computation of hopping for next event
                    lld_ci_compute_hop_scheme(p_cis, p_cis->hop_ptr[hop_tog], p_cis->cis_evt_cnt + p_cis->evt_inc + 1);

                    // Store new value of hopping toggle bit
                    SETB(p_cis->cis_info, LLD_CIS_INFO_HOP_TOG, hop_tog);
                }
                             ...
                             uint32_t timestamp, bool abort)
{
    ...
    while(!cig_stop && (p_cig->in_proc_se_idx < p_cig->prog_se_idx))
    {
        ...
        {
            p_cis = LLD_CIS_GET(p_skip_se->chan_act_id);
            cig_stop = lld_ci_end_subevt(p_cis, p_cig, true);
        }
 ...
 * Hence the end interrupt following the skip interrupt will not be for the skipped channel
 * or the CIG the skip channel belongs to.
 ****************************************************************************************
 */
                                ...
                                uint32_t timestamp)
{
    ...
    if (p_rx->max_pdu > 0)
    {
        ...
        lld_isoal_rx_done(p_cis->act_id, p_data->buf_idx, p_cig->bts_array[bts_idx], false);
...
__STATIC void lld_ci_frm_cbk(uint32_t timestamp, uint32_t dummy, uint8_t irq_type)
{
    ...
    // Get CIS structure
    struct lld_cis_env* p_cis       = LLD_CIS_GET((uint8_t)GETF(dummy, LLD_CI_DUMMY_CHAN_HDL));
    // Get CIG structure
...
void lld_ci_init(uint8_t init_type)
{
    switch (init_type)
    {
        case RWIP_RST:
        {
            ...
            {
                struct lld_cig_env *p_cig = (struct lld_cig_env *)lld_iso_env.p_groups[cnt];

                if (p_cig && (lld_iso_env.grp_type[cnt] == LLD_ISO_MODE_CI))
                {
                    // Clean the structure
                    lld_cig_free(p_cig);
                }
            ...
            {
                struct lld_cis_env *p_cis = (struct lld_cis_env *)lld_iso_env.p_channels[cnt];

                if (p_cis && (lld_iso_env.chan_type[cnt] == LLD_ISO_MODE_CI))
                {
                    // Clean the structure
    ...
    do
    {
        ...
        if(p_cis_params->rx_max_sdu > 0)
        {
            ...
            if ((p_cis_params->framing == ISO_UNFRAMED_MODE) && (p_cis_params->role == MASTER_ROLE))
            {
                 ...
                 *  - CIG_Sync_Delay - ((ISO_Interval ÷ SDU interval)-1) × SDU interval
                 */
                sync_ref_offset = -p_cis_params->cis_spacing - p_cis_params->iso_interval + p_cis_params->rx_sdu_interval;
            }
            // Unframed / slave
            else if ((p_cis_params->framing == ISO_UNFRAMED_MODE) && (p_cis_params->role == SLAVE_ROLE))
            {
                /*
                 * SDU_Synchronization_Reference = CIS reference anchor point + CIS_Sync_Delay
                 *  + (FT_M_To_S - 1) × ISO_Interval
                 */
                sync_ref_offset = p_cis_params->cis_sync_delay + ((p_cis_params->rx_ft - 1)*p_cis_params->iso_interval);
            }
            // Framed / master
            ...
            else if ((p_cis_params->framing == ISO_FRAMED_MODE) && (p_cis_params->role == SLAVE_ROLE))
            {
                /*
                 * SDU_Synchronization_Reference = CIS Reference Anchor point + CIS_Sync_Delay
                 *  + SDU_Interval_M_To_S + FT_M_To_S × ISO_Interval - Time_Offset
                 */
        ...
        // ISO link control
        em_ble_isolinkcntl_pack(cs_idx, /*streamlbl*/  p_cis_params->chan_hdl,
        ...
        for (cnt = 0; cnt < BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN; cnt++)
        {
            ...
            uint8_t next_isodesc_idx = (cnt < (BLE_NB_RXTX_ISO_DESC_BUF_PER_CIS_CHAN - 1))
        ...
        // Request alarm to be started only if new channel cannot be insert automatically by a start event
        if(p_cig->nb_cis == 1)
        {
            // Get pointer to alarm
            struct sch_alarm_tag *p_alarm = &p_cig->alarm;
        ...
        // else channel will be started during end of CIG handling as it is not marked as STARTED
...
uint8_t lld_cis_stop(uint8_t act_id)
{
    ...
    if (p_cis != NULL)
    {
        ...
        // if CIG is not in progress and if there is one CIS only in the CIG, activity can be immediately stopped
        if (!GETB(p_cig->cig_info, LLD_CIG_INFO_PROG) && (p_cig->nb_cis == 1))
        {
            // Remove the event from the EA
            sch_arb_remove(&p_cig->evt, false);

            // Channel can be removed now
            lld_cis_remove(p_cig, p_cis);
        ...
        {
            // Keep in mind that channel has to be stopped when CIG event will be over
...
void lld_ci_ch_map_update(uint8_t link_id, struct le_chnl_map *p_map, uint32_t ref_timestamp)
{
    ...
    for(act_id = 0 ; act_id < BLE_ACTIVITY_MAX ; act_id++)
    {
        ...
        // Check if indicated channel exists and CIS managed by specific link
...
void lld_ci_sync_time_update(uint8_t link_id, uint32_t last_sync_ts, int32_t sync_drift_acc)
{
    ...
    {
        p_cig = (struct lld_cig_env *)lld_iso_env.p_groups[cnt];

        // Check if group is a CIG, link ID matches, and CIG event is not ongoing
        if (p_cig && (lld_iso_env.grp_type[cnt] == LLD_ISO_MODE_CI) && (p_cig->con_link_id == link_id) && !GETB(p_cig->cig_info, LLD_CIG_INFO_PROG))
        {
            // Get drift since last drift value
            int32_t drift_val = sync_drift_acc - p_cig->last_drift_acc;
            // CIG anchor timestamp - half microseconds part
            ...
            p_cig->last_drift_acc = sync_drift_acc;
