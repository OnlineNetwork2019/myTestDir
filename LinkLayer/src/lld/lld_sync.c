...
/// Calculate drift in ppm from local drift and SyncInfo SCA field
#define SYNC_DRIFT_PPM(local_drift, sca) ((local_drift) + (co_sca2ppm[sca]))
...
struct lld_sync_env_tag* lld_sync_env[BLE_ACTIVITY_MAX];
...
__STATIC void lld_sync_process_pkt_rx(uint8_t act_id)
{
    ...
    while (lld_rxdesc_check(EM_BLE_CS_ACT_ID_TO_INDEX(act_id)))
    {
        ...
        if((rxstat_pkt & LLD_ADV_ERR_MASK) == 0)
        {
            ...
            data_len = em_ble_rxphadv_rxadvlen_getf(rxdesc_idx) - rxaelength - BLE_EXT_ADV_PRE_HEADER_LEN;
            ...
            if (rxaelength > 0)
            {
                ...
                if (rxaeheader & EM_BLE_RXAUXPTR_BIT) // AuxPtr
                {
                    ...
                    if (0 == em_ble_rxstatadv_followauxptr_getf(rxdesc_idx))
                    {
                            ...
                            lld_calc_aux_rx(&sync_env->aux_rx_out, rxdesc_idx, aux_data))
                        {
                            ...
                            sync_env->data_status = ADV_EVT_DATA_STATUS_INCOMPLETE;
                        }
                        else
                        {
                            // No chained packet will follow, indicate as truncated.
                            sync_env->aux_offload = NO_AUX_OFFLOAD;
                            sync_env->data_status = ADV_EVT_DATA_STATUS_TRUNCATED;
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    sync_env->data_status = ADV_EVT_DATA_STATUS_COMPLETE;
                }
...
{
    // Point to parameters
    struct lld_sync_env_tag* sync_par = lld_sync_env[act_id];

    uint8_t cs_idx = EM_BLE_CS_ACT_ID_TO_INDEX(act_id);

    // Check if CTE is enabled
    if (sync_par->cte_params.sampl_en)
    {
        // Set the Antenna ID pointer
        em_ble_rxdfantswptr_set(cs_idx, EM_BLE_RX_ANTENNA_ID_OFFSET >> 2);

        // Write antenna IDs to EM
        em_wr(&sync_par->cte_params.antenna_id[0], EM_BLE_RX_ANTENNA_ID_OFFSET, sync_par->cte_params.switching_pattern_len);

        // Set the length of the switching pattern
        em_ble_rxdfantpattcntl_rx_ant_patt_length_setf(cs_idx, sync_par->cte_params.switching_pattern_len);

        // max I&Q samples per CTE
        em_ble_crcinit1_rxmaxctebuf_setf(cs_idx, LLD_MAX_CTE_IQ_SAMPLES);

        // max sampled CTE per event
        if (sync_par->cte_params.max_sampl_cte != 0)
            em_ble_rxdfantpattcntl_max_samp_cte_setf(cs_idx, sync_par->cte_params.max_sampl_cte);
        else
        ...
        em_ble_rxdfcntl_pack(cs_idx, /*DFRSPEN*/ 0, /*DFSWCNTL*/ sync_par->cte_params.slot_dur, /*DFSAMPCNTL*/ sync_par->cte_params.slot_dur, /*DFTYPE*/ 0x03, /*DFFILTEREN*/ 0, /*DFEN*/ 1);
    }
    else
    {
...
__STATIC void lld_sync_sched(uint8_t act_id, bool resched)
{
    ...
    while (!elt_inserted)
    {
        ...
        if(resched)
        {
            evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_PER_ADV_RX_DFT_IDX);
        }
        else
        {
            evt->current_prio = rwip_priority[RWIP_PRIO_PER_ADV_RX_DFT_IDX].value;
        }
        ...
        else
        {
            ...
            if((sync_env->pkt_rx > 0) && (ADV_EVT_DATA_STATUS_COMPLETE == sync_env->data_status) && (sync_env->skip != 0))
            {
                // First scheduling
                if(!resched)
                {
            ...
            if(clk_diff > 0)
            {
                // check that we are  trying to go backward in skipping and we cannot go further in the past
                if(!BLE_UTIL_INSTANT_PASSED(sync_env->skip_end_evt_cnt, sync_env->evt_counter))
                {
                    // ensure that we increment event counter just after the event skip
            ...
            // compute max drift, avoid uint32 overflow in calculation
            max_drift_us = rwip_active_check() ? lld_env.local_drift_active : lld_env.local_drift;
            max_drift_us = ((CLK_SUB(sync_env->anchor_ts, sync_env->last_sync_ts)/16)
                         * SYNC_DRIFT_PPM(max_drift_us, sync_env->adv_ca)) / ((1000000/HALF_SLOT_SIZE)/16);

            // Calculate the new Rx window size
            sync_win_size_us = (2*sync_env->add_drift/2) + 2*max_drift_us + 2*BLE_MAX_JITTER;
            ...
            // Compute the new bit offset
            {
                ...
                uint32_t half_win_size = (sync_win_size_us >> 1);
        ...
        // Reserve duration large enough to receive AUX_SYNC_IND, and option for HW Follow of packed chained data.
        evt->duration_min = (lld_sync_max_aux_dur_tab[rate] << 1) + (sync_win_size_us << 1) + BLE_RESERVATION_TIME_MARGIN_HUS;

        // Bound event duration in event large sync_win extends duration_min to exceed scheduling arbitration accepted range
        if (evt->duration_min > SCH_ARB_MAX_DURATION)
        {
            evt->duration_min = SCH_ARB_MAX_DURATION;
        }
        ...
        if (sch_arb_insert(evt) == SCH_ARB_ERROR_OK)
        {
            ...
            max_evt_slots = CO_DIVIDE_CEIL((evt->duration_min>>1) - BLE_RESERVATION_TIME_MARGIN_HUS, SLOT_SIZE);
...
__STATIC void lld_sync_frm_eof_isr(uint8_t act_id, uint32_t timestamp, bool abort)
{
    ...
    if(lld_sync_env[act_id] != NULL)
    {
        ...
        if (em_ble_txrxcntl_rxmafserr_getf(cs_idx))
        {
            ...
            if (ADV_EVT_DATA_STATUS_INCOMPLETE == sync_env->data_status)
            {
                // Incomplete advertisement, generate a truncated report
                lld_sync_trunc_ind(act_id);
            }
        ...
        if (((HW_CNTL_AUX_OFFLOAD == sync_env->aux_offload) || (!sync_env->pkt_rx && (SW_CNTL_AUX_OFFLOAD == sync_env->aux_offload)))
                && (ADV_EVT_DATA_STATUS_INCOMPLETE == sync_env->data_status))
        {
            // Incomplete advertisement, generate a truncated report
            lld_sync_trunc_ind(act_id);
...
uint8_t lld_sync_start(uint8_t act_id, struct lld_sync_params* params)
{
    ...
    if ((act_id < BLE_ACTIVITY_MAX) && (NULL == lld_sync_env[act_id]))
    {
        ...
        if (lld_sync_env[act_id] != NULL)
        {
            ...
            em_ble_cntl_pack(cs_idx, RWIP_COEX_GET(SCAN, TXBSY), RWIP_COEX_GET(SCAN, RXBSY), RWIP_COEX_GET(SCAN, DNABORT), EM_BLE_CS_FMT_EXT_PASSIVE_SCAN);
...
uint8_t lld_sync_info_for_big_get(uint8_t act_id, uint16_t evt_cnt, struct lld_sync_info_for_big* info)
{
    ...
    if(sync_par != NULL)
    {
        ...
        sync_evt_cnt          = sync_par->evt_counter;
        sync_interval         = sync_par->intv;
        sync_timestamp        = sync_par->anchor_ts;
        ...
        info->last_sync_ts   = sync_par->last_sync_ts;
        info->ref_bit_off    = sync_par->anchor_bit_off;
        info->sync_drift_acc = 0; //  sync_par->sync_drift_acc;
        info->ca             = sync_par->adv_ca;
