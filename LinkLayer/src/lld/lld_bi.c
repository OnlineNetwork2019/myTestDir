...
/// Distance between scheduling alarm and timestamp of scheduling event (in half slots)
#define LLD_BI_ALARM_DISTANCE                  (6)
/// Minimum distance between end of scheduling and beginning of next one for use of scheduling alarm (in half slots)
#define LLD_BI_ALARM_MIN_DISTANCE              (LLD_BI_ALARM_DISTANCE + 4)
...
#define LLD_BI_SYNC_WIN_MIN_DISTANCE           (1000)
...
/// Get BIS structure
#define LLD_BIS_GET(act_id)     ((struct lld_bis_env *)lld_iso_env.p_channels[act_id])
/// Get BIG structure
#define LLD_BIG_GET(grp_hdl)    ((struct lld_big_env *)lld_iso_env.p_groups[grp_hdl])
...
enum lld_bis_info_fields
{
    ...
    /// Indicate if one subevent has been programmed for this channel event
    LLD_BIS_INFO_PROG_POS        = 1,
    LLD_BIS_INFO_PROG_BIT        = 0x02,

    /// Indicate that channel has been started
...
///    7       6      5         4          3          2         1        0
/// +------+------+-------+-----------+---------+------------+------+---------+
/// | INIT | SYNC | SLAVE | ALARM PROG| NEW_EVT | INTERLACED | PROG | ENCRYPT |
/// +------+------+-------+-----------+---------+------------+------+---------+
///
...
enum lld_big_info_fields
{
    ...
    /// Indicate that BIG event is in progress, cannot add/remove a channel and
    /// cannot update scheduling information
    LLD_BIG_PROG_POS          = 1,
    LLD_BIG_PROG_BIT          = 0x0002,

    /// Indicate that streams are interlaced
    LLD_BIG_INTERLACED_POS    = 2,
    LLD_BIG_INTERLACED_BIT    = 0x0004,

    /// Indicate that no sub-event for the BIG event has been programmed yet.
    /// If this bit is still set to 1 when BIG event is done, means that the BIG event has been
    /// entirely skipped.
    LLD_BIG_NEW_EVT_POS       = 3,
    LLD_BIG_NEW_EVT_BIT       = 0x0008,

    /// Indicate that scheduling alarm is for programming new sub-events
    LLD_BIG_ALARM_PROG_POS    = 4,
    LLD_BIG_ALARM_PROG_BIT    = 0x0010,

    /// Indicate if we are slave of BIG
    LLD_BIG_SLAVE_POS         = 5,
    LLD_BIG_SLAVE_BIT         = 0x0020,

    /// Indicate that synchronization has been caught during a scheduling event (slave only)
    LLD_BIG_SYNC_POS          = 6,
    ...
    /// Indicates if BIG sync has been established (slave only)
...
/// +---------+-------------+----------+---------+
/// |   TYPE  | SUB_EVT_IDX | CHAN_HDL | GRP_HDL |
/// +---------+-------------+----------+---------+
///
enum lld_bi_dummy_fields
{
    ...
    /// Channel Handle
    LLD_BI_DUMMY_CHAN_HDL_LSB          = 8,
    LLD_BI_DUMMY_CHAN_HDL_MASK         = 0x0000FF00,
...
struct lld_big_env
{
    ...
    /// List of BIS belonging to this BIG (sorted by channel offset values)
    ...
    /// Supervision timeout (in half microseconds)
    uint32_t                         timeout;
    #endif //(BLE_OBSERVER)
    ...
    uint32_t                         anchor_ts;
    /// ISO Interval (in microseconds) - all channels have the same channel interval
    uint32_t                         iso_interval_us;
    /// Group anchor timestamp - half microseconds part
    uint16_t                         anchor_bit_off;
    /// ISO Interval (in half slots) - all channels have the same channel interval
    ...
    /// Event counter of the first BIG event the device attempted to receive (slave only)
    ...
    /// Stop reason
    uint8_t                          stop_reason;
    /// Number of subevents per channel (Range 0x01-0x1F)
    uint8_t                          bis_nse;
    /// Maximum PDU size (in bytes)
    uint8_t                          max_pdu;
    /// Number of new packet per ISO interval (Burst Number) (0: no new packet -  Range 0x01-0x1F)
    uint8_t                          bn;
    /// Number of monitored data packets for each channel
    /// (provide number of elements in @see lld_bis_env p_data_array)
    uint8_t                          nb_data_pkt;
    /// Index in @see lld_bis_env p_data_array of first packet handled during channel event
    uint8_t                          data_pos;
...
struct lld_bis_env
{
    ...
    /// Channel offset (in half microseconds) - Time from the BIG anchor to BIS anchor
    ...
    /// Channel Activity identifier
    uint8_t                          act_id;
    /// Channel information (@see enum lld_bis_info_fields)
    uint8_t                          bis_info;
    ...
    uint8_t                          iso_descs_idx[BLE_NB_ISODESC_PER_BIS_CHAN];
    ...
    // -- BIS Data Specific
    /// Pointer to data structures used for this channel - Index in this array provides the index
...
/**
 ****************************************************************************************
 * @brief Free an BIS structure after channel stop request. This function stops the
 * data path if started and free the ISO descriptors allocated for the link.
 *
 * @param[in] p_bis        Pointer to the BIS structure
 * @param[in] stop_reason  Reason why the channel has be stopped
 * @param[in] nb_data_pkt  Number of monitored data packets for each channel
 * @param[in] reset        Used to know if a reset is on-going
 ****************************************************************************************
 */
__STATIC void lld_bis_free_channel(struct lld_bis_env *p_bis, uint8_t stop_reason, uint8_t nb_data_pkt, bool reset)
{
    if(!reset)
    {
        // Counter
        ...
        for (cnt = 0; cnt < BLE_NB_ISODESC_PER_BIS_CHAN; cnt++)
        {
        ...
        // Update LLD BIS environment
        lld_iso_env.p_channels[p_bis->act_id] = NULL;
...
__STATIC void lld_big_free(struct lld_big_env *p_big, bool indicate, bool reset)
{
    ...
    if (indicate)
    {
        // Send the lld_big_sync_estab_ind with status "failed to be established" only when slave and sync not yet established
        // In all other cases send the lld_big_stop_ind
        if (!GETB(p_big->big_info, LLD_BIG_SLAVE) || GETB(p_big->big_info, LLD_BIG_SYNC_ESTAB))
        {
            // Inform link layer controller that driver is stopped for the provided channel
    ...
    lld_iso_env.p_groups[p_big->grp_hdl] = NULL;
...
/**
 ****************************************************************************************
 * @brief Set scheduling information for all sub events within a BIG (interlaced case)
 *
...
__STATIC void lld_big_compute_sched_int(struct lld_big_env *p_big)
{
    ...
    while (true)
    {
        ...
        // Get first channel information
        p_bis = (struct lld_bis_env *)co_list_pick(&p_big->list_bis);

        while (p_bis && (p_bis->chan_idx != 0))
        {
            // Check if channel has been started
            if (GETB(p_bis->bis_info, LLD_BIS_INFO_STARTED))
            {
                if (sub_evt_cnt < p_bis->hop_inf.nse)
                {
                    ...
                    p_se->chan_act_id     = p_bis->act_id;
                    p_se->offset          = p_bis->chan_offset + (sub_evt_cnt * p_bis->sub_interval);
...

    {
        ...
        // Sanity check
        ASSERT_ERR(p_bis->chan_idx == 0);

        p_se->chan_act_id     = p_bis->act_id;
        p_se->offset          = p_bis->chan_offset;
    ...
    while (p_bis)
    {
        ...
        if (GETB(p_bis->bis_info, LLD_BIS_INFO_STARTED))
        {
            ...
            for (se_cnt = 0; se_cnt < p_bis->hop_inf.nse; se_cnt++)
            {
                ...
                p_se->offset          = p_bis->chan_offset + (se_cnt * p_bis->sub_interval);
                p_se->state           = 0;
                p_se->nb_packed       = 0;
...
/**
 ****************************************************************************************
 * @brief Compute scheduling information after introduction or deletion of a channel for
 * a given BIG
 *
 * @param[in] p_big       Pointer to the BIG structure
 *
...
__STATIC uint8_t lld_big_compute_sched(struct lld_big_env *p_big)
{
    ...
    p_big->p_se_infos = ke_malloc(sizeof(struct lld_bi_se) * p_big->big_nse, KE_MEM_ENV);
    ...
    if (p_big->p_se_infos)
    {
        ...
        if (GETB(p_big->big_info, LLD_BIG_INTERLACED))
        {
        ...
        p_bis    = LLD_BIS_GET(p_se->chan_act_id);
 ...
 * D = ((35 × N) + 42) MOD 128 = D7D6D5D4D3D2D1D0.
 * and set diversifier word as
 * DW = D0D0D0D0 D0D0D1D6 D1b0D5D4 b0D3D2b0 00000000 00000000b
 *
 * AA = DW XOR SAA
 *
 * @param[in] chan_idx         BIS index (between 1 and n, with n = number of channels in the BIG)
 * @param[in] seed_acc_addr    Seed access address of the Group
 *
 * @return Access address to be used for the BIS
 ****************************************************************************************
 */
__STATIC uint32_t lld_bis_aa_gen(uint8_t chan_idx, uint32_t seed_acc_addr)
{
    // diversifier ((35 × N) + 42) MOD 128
    uint8_t d = ((35 * chan_idx) + 42) & 0x7F;
    // diversifier word
...
/**
 ****************************************************************************************
 * @brief Insert a channel BIS structure in the list of channel for a given BIG.
 * Channel elements are sorted by channel offset values
 *
 * @param[in] p_bis     BIS structure
 * @param[in] p_big     BIG structure
 ****************************************************************************************
 */
__STATIC void lld_bis_insert_channel(struct lld_bis_env *p_bis, struct lld_big_env *p_big)
{
    // Get first channel element
    struct lld_bis_env *p_list_channel = (struct lld_bis_env *)co_list_pick(&p_big->list_bis);

    if (!p_list_channel)
    {
        // Insert the channel at the beginning of the list
        co_list_push_front(&p_big->list_bis, &p_bis->hdr);
    }
    else
    {
        while (p_list_channel)
        {
            if (p_bis->chan_offset <= p_list_channel->chan_offset)
            {
                // Insert the channel
                co_list_insert_before(&p_big->list_bis, &p_list_channel->hdr, &p_bis->hdr);
                break;
            }

            // Get next element
            p_list_channel = (struct lld_bis_env *)(p_list_channel->hdr.next);
        }

        if (!p_list_channel)
        {
            // Insert the channel at the end of the list
 ...
 * @param[in] p_big_info     Pointer to BIG info (used to generate ACAD data on master side)
...
__STATIC struct lld_big_env *lld_big_alloc(struct lld_big_params *p_params, struct big_info *p_big_info, uint8_t nb_pt_evt)
{
    ...
    struct lld_big_env *p_big = (struct lld_big_env *)ke_malloc(sizeof(struct lld_big_env) +
    ...
    if (p_big)
    {
        do
        {
            ...
            p_big->prio_idx          = (p_params->role == MASTER_ROLE) ? RWIP_PRIO_M_BIS_IDX : RWIP_PRIO_S_BIS_IDX;

            p_big->big_info          = 0;
            SETB(p_big->big_info, LLD_BIG_ENCRYPT,    p_big_info->encrypted);
            SETB(p_big->big_info, LLD_BIG_SLAVE,      (p_params->role == ROLE_SLAVE));
            SETB(p_big->big_info, LLD_BIG_NEW_EVT,    1);
            SETB(p_big->big_info, LLD_BIG_INTERLACED, (p_params->packing == ISO_PACKING_INTERLEAVED));
            SETB(p_big->big_info, LLD_BIG_HOP_TOG,    0);
            ...
            // Update ISO environment
            lld_iso_env.p_groups[p_params->grp_hdl] = p_big;
            lld_iso_env.grp_type[p_params->grp_hdl] = LLD_ISO_MODE_BI;
            ...
            #endif //(BLE_BROADCASTER)
            {
                ...
                big_evt_cnt           = ((co_read24p(&p_big_info->bis_pkt_cnt[3]) & 0x7FFFFF) / p_big_info->bn) << 24;
                // keep rest of the previous euclidian function before doing division of LSB
                big_evt_cnt          += ( (CO_MOD((co_read24p(&p_big_info->bis_pkt_cnt[3]) & 0x7FFFFF), p_big_info->bn) << 24)
...
{
    // Is this channel used for control sub-event ?
    bool control_se = (p_bis_params->chan_idx == 0);

    // Number of data packets
    uint8_t nb_data_pkt = (control_se) ? 1 : p_big->nb_data_pkt;

    // Allocate a new BIS structure
    struct lld_bis_env *p_bis = (struct lld_bis_env *)ke_malloc(sizeof(struct lld_bis_env) +
    ...
    if (p_bis != NULL)
    {
        ...
        p_bis->act_id   = p_bis_params->chan_act_id;
        p_bis->grp_hdl  = p_big_params->grp_hdl;
        p_bis->chan_idx = p_bis_params->chan_idx;

        #if (BLE_BROADCASTER)
        p_bis->data_init_mask = CO_BIT(nb_data_pkt) - 1;
        #endif // (BLE_BROADCASTER)

        // Compute the access address to use for BIS
        p_bis->hop_inf.acc_code = lld_bis_aa_gen(p_bis_params->chan_idx, seed_acc_addr);
        ...
        else
        {
            ...
            p_bis->chan_offset    = (p_bis_params->chan_idx - 1) * US_TO_HUS(bis_spacing);
            #if (BLE_OBSERVER)
        ...
        // Consider the channel as started
        SETB(p_bis->bis_info, LLD_BIS_INFO_STARTED, 1);

        // Update ISO environment
        lld_iso_env.p_channels[p_bis_params->chan_act_id] = p_bis;
        lld_iso_env.chan_type[p_bis_params->chan_act_id]  = LLD_ISO_MODE_BI;

        // Insert the channel in the list of channel for the provided group
        lld_bis_insert_channel(p_bis, p_big);
    }
...
__STATIC void lld_bi_compute_hop_scheme(struct lld_big_env *p_big, uint16_t evt_cnt)
{
    ...
    while(p_bis != NULL)
    {
        if(p_bis->chan_idx != 0)
        {
            ...
            lld_iso_compute_hop_scheme(&(p_bis->hop_inf));
        }
        p_bis = (struct lld_bis_env*) p_bis->hdr.next;
    }
}
                                  ...
                                  uint8_t data_offset)
{
    ...
    if(GETB(p_big->big_info, LLD_BIG_SLAVE))
    {
        lld_isoal_rx_done(p_bis->act_id, p_data->buf_idx, p_big->anchor_bts, !GETB(p_data->data_info, LLD_BIS_DATA_PROG));
    }
    else
    #endif //(BLE_OBSERVER)
    {
        ...
        if(!GETB(p_data->data_info, LLD_BIS_DATA_PROG))
        {
            lld_isoal_tx_get(p_bis->act_id, p_data->buf_idx, p_big->anchor_bts, true);
        }
        #endif //(BLE_BROADCASTER)
    }
                                ...
                                struct lld_bi_se *p_se, uint8_t offset)
{
    ...
    do
    {
        ...
        if (GETB(p_big->big_info, LLD_BIG_SLAVE))
        {
            ...
            // Set Buffer pointer
            em_ble_rxisobufptr_setf(idx, ((p_bis->chan_idx) ? ble_util_buf_iso_emptr_get(p_data->buf_idx) : 0) >> 2);
        ...
        #endif //(BLE_OBSERVER)
        {
            ...
            // Set Buffer pointer
            em_ble_txisobufptr_setf(idx, ((p_bis->chan_idx) ? ble_util_buf_iso_emptr_get(p_data->buf_idx)
        ...
        if (p_bis->chan_idx)
        {
            ...
            CO_VAL_INC(p_bis->next_desc_idx, BLE_NB_ISODESC_PER_BIS_CHAN);
        }
    } while (0);
}
...
__STATIC void lld_bi_alarm_cbk(struct sch_alarm_tag *p_alarm)
{
    ...
    if(GETB(p_big->big_info, LLD_BIG_ALARM_PROG))
    {
        ...
        struct lld_bis_env* p_bis = LLD_BIS_GET(p_se->chan_act_id);
        ...
        if(p_big->prog_se_idx < (p_big->ref_se_idx + p_ref_se->nb_packed))
        {
            ...
            if(GETB(p_big->big_info, LLD_BIG_SLAVE) && GETB(p_big->big_info, LLD_BIG_SYNC))
            {
                // Timings are computed from the originally expected synchronization and the timings from last synchronization
                bit_off = p_next_se->exp_sync_bit_off + p_big->slave_offset - BLE_NORMAL_WIN_SIZE/2;
    ...
    else if(GETB(p_big->big_info, LLD_BIG_SCHED_REJECT))
    {
        ...
        {
            // Get channel handle for this subevent
            uint8_t chan_hdl = p_big->p_se_infos[cnt].chan_act_id;

            // Handle end of sub event
            lld_bi_end_subevt(LLD_BIS_GET(chan_hdl), p_big, LLD_BI_SE_STATUS_REJ_BIT);
        }
...
__STATIC void lld_bi_sched(struct lld_big_env *p_big, uint32_t clock)
{
    ...
    do
    {
        ...
        #if (BLE_OBSERVER)
        // Check if we are slave for this channel, if yes we have to take care of
        // drift with master device and rx sync window.
        if (GETB(p_big->big_info, LLD_BIG_SLAVE))
        {
            // first element to get sub-event interval
            struct lld_bis_env *p_bis = (struct lld_bis_env *)co_list_pick(&p_big->list_bis);
            // Local drift and peer drift in PPM
            uint16_t local_drift = rwip_active_check() ? lld_env.local_drift_active : lld_env.local_drift;
            uint16_t peer_drift = p_big->sca;
            // Duration between last synchronization and next expected anchor in half slots
            uint32_t clock_diff_hs = CLK_DIFF(p_big->last_sync_ts, target_clock);
            ...
            uint32_t win_widening = max_drift + 2*US_TO_HUS(BLE_MAX_JITTER);
            // Computed RX window size - will be the same for all subevents inside the scheduling event
            uint32_t rx_win_size = 2*win_widening + p_big->def_rx_win_size;
            // Window widening limit, after which a disconnection occurs
            uint32_t win_lim = (p_bis->hop_inf.nse < 3) ? (p_big->iso_interval*HALF_SLOT_SIZE/2 - US_TO_HUS(BLE_IFS_DUR)) : p_bis->sub_interval;

            // Check if window is large enough
            if (rx_win_size < US_TO_HUS(BLE_NORMAL_WIN_SIZE))
            {
                rx_win_size = US_TO_HUS(BLE_NORMAL_WIN_SIZE);
            }

            // Disconnect if the window widening value has reached its limit
            if(win_widening >= win_lim)
            {
                timeout = true;
                // Here we have a synchronization timeout
                break;
            }
            ...
            // Compute the new bit offset, RX windows centered on expected anchor point
            // Remove half of the window
            bit_off -= rx_win_size / 2;

            // Check if we are on previous slot
            while (bit_off < 0)
            {
                target_clock = CLK_SUB(target_clock, 1);
                bit_off += HALF_SLOT_SIZE;
            ...
            // Keep the RX window size
            p_big->rx_win_size = rx_win_size;
        }
        #endif //(BLE_OBSERVER)
        ...
        if (CLK_GREATER_THAN(p_first_se->timestamp, clock))
        {
            ...
            // Ensure that 1 half slot is used at least
            if(p_evt->duration_min < HALF_SLOT_SIZE)
            {
                p_evt->duration_min = HALF_SLOT_SIZE;
            }
        ...
        p_evt->current_prio += RWIP_PRIO_INC(p_big->prio_idx);
...
/**
 ****************************************************************************************
 * @brief Link RX or TX ISO descriptors together for a given channel. Mark them as done.
 *
 * @param[in] p_bis    Pointer to the BIS structure
 * @param[in] p_big    Pointer to the BIG structure
 ****************************************************************************************
...
__STATIC void lld_bis_configure_descs(struct lld_bis_env *p_bis, struct lld_big_env *p_big)
{
    ...
    uint8_t nb_desc = (p_bis->chan_idx) ? BLE_NB_ISODESC_PER_BIS_CHAN : 1;
...
/**
 ****************************************************************************************
 * @brief Fill the control structure used by a given channel for BIS data transmission or
 * reception. Done once when channel is started.
 *
 * @param[in] p_bis         Pointer to the BIS structure.
 * @param[in] p_big         Pointer to the BIG structure.
...
__STATIC void lld_bi_fill_cs(struct lld_bis_env *p_bis, struct lld_big_env *p_big,
                             struct lld_big_params *p_bis_params, uint16_t crc_init, uint8_t* giv)
{
    // CS Index
    uint8_t cs_idx = EM_BLE_CS_ACT_ID_TO_INDEX(p_bis->act_id);
    ...
    em_ble_cntl_pack(cs_idx, RWIP_COEX_GET(CON, TXBSY),
                             ...
                             ((p_bis_params->role == ROLE_MASTER)
                                     ? EM_BLE_CS_FMT_MST_CONNECT : EM_BLE_CS_FMT_SLV_CONNECT));

    // ISO link control
    em_ble_isolinkcntl_pack(cs_idx, /*streamlbl*/   p_bis->chan_idx,
    ...
    // Link control
    em_ble_linkcntl_pack(cs_idx, /*linklbl*/       cs_idx,
                                 /*sas*/           false,
                                 /*nullrxllidflt*/ true,
     ...
     */
    em_ble_crcinit0_set(cs_idx, ((crc_init & 0xFF) << 8) | p_bis->chan_idx);
    em_ble_crcinit1_pack(cs_idx, /*rxmaxctebuf*/ 0, /*crcinit1*/ (crc_init >> 8));

    // Hopping control
    em_ble_hopcntl_pack(cs_idx, /*fhen*/     true,
                                /*hop_mode*/ (p_bis->chan_idx) ? LLD_HOP_MODE_SEQ_PTR : LLD_HOP_MODE_CHAN_SEL_2,
    ...
    #if (BLE_OBSERVER && BLE_BROADCASTER)
    if (p_bis_params->role == ROLE_SLAVE)
    #endif //(BLE_OBSERVER && BLE_BROADCASTER)
    {
        #if (BLE_OBSERVER)
        // Set Rx Max buf and Rx Max Time + ISO receive payload length authorized
        uint8_t pkt_len = ((p_bis->chan_idx == 0) ? LE_MIN_OCTETS : p_big->max_pdu) + (encrypt ? MIC_LEN : 0);

        em_ble_rxmaxbuf_pack(cs_idx, pkt_len, pkt_len);
        em_ble_rxmaxtime_set(cs_idx, LE_MAX_TIME);

        // Set the normal win size
        em_ble_rxwincntl_pack(cs_idx, 0, BLE_NORMAL_WIN_SIZE);
        #endif //(BLE_OBSERVER)
    ...
    #if (BLE_OBSERVER && BLE_BROADCASTER)
    if (p_bis_params->role == SLAVE_ROLE)
    #endif //(BLE_OBSERVER && BLE_BROADCASTER)
    {
        #if (BLE_OBSERVER)
...
__STATIC void lld_bi_prog(struct lld_big_env *p_big, uint8_t current_prio)
{
    ...
    do
    {
        ...
        else
        {
            ...
            type = ((p_big->nb_cntl_tx == 0) && !GETB(p_big->big_info, LLD_BIG_SLAVE))
        ...
        if (   (  !GETB(p_big->big_info, LLD_BIG_PROG_EN)
             ...
             #if (BLE_OBSERVER)
             ||    ( GETB(p_big->big_info, LLD_BIG_SLAVE)
        ...
        prog_par.add.ble.sic    = GETB(p_big->big_info, LLD_BIG_SLAVE) && !GETB(p_big->big_info, LLD_BIG_PROG_SYNC) && GETB(p_big->big_info, LLD_BIG_PROG);
        prog_par.mode           = SCH_PROG_BLE;

        // Update priority for Scanning sub-event
        if(   (p_bis->chan_idx == 0) && !GETB(p_big->big_info, LLD_BIG_SLAVE)
           && (p_big->nb_cntl_tx == 0))
        {
        ...
        // Fill dummy value
        prog_par.dummy = 0;
        SETF(prog_par.dummy, LLD_BI_DUMMY_CHAN_HDL,    p_bis->act_id);
        ...
        #if (BLE_OBSERVER)
        // Set sync window size in the control structure if we are slave
        if (GETB(p_big->big_info, LLD_BIG_SLAVE))
        {
            // Get synchronization window size in microseconds
            uint32_t sync_win_size_us = GETB(p_big->big_info, LLD_BIG_PROG_SYNC) ? BLE_NORMAL_WIN_SIZE : HUS_TO_US(p_big->rx_win_size + 1);
        ...
        // If first sub-event programmed for the channel event, prepare the control structure
        if (!GETB(p_bis->bis_info, LLD_BIS_INFO_PROG))
        {
            ...
            // Keep in mind that the CS has been filled for the channel event
 ...
 ****************************************************************************************
 */
__STATIC void lld_bis_end_evt(struct lld_bis_env *p_bis, struct lld_big_env *p_big)
{
    // Counter
    uint8_t cnt;
    ...
    if (p_bis->chan_idx)
    {
        ...
        #if (BLE_OBSERVER)
        // Slave, check if supervision timeout reached
        if(GETB(p_big->big_info, LLD_BIG_SLAVE))
        {
            ...
            {
                p_big->stop_reason = CO_ERROR_CON_TIMEOUT;
                SETB(p_big->big_info, LLD_BIG_STOP, true);
            }
        }
        #endif // (BLE_OBSERVER)
    }
    #if (BLE_BROADCASTER)
    // Broacaster and Control sub-event in use
    else if (!GETB(p_big->big_info, LLD_BIG_SLAVE) && (p_big->nb_cntl_tx > 0) && (!GETB(p_big->big_info, LLD_BIG_CSE_CS_UPDATE)))
    {
...
__STATIC bool lld_big_end_evt(struct lld_big_env *p_big)
{
    ...
    if (GETB(p_big->big_info, LLD_BIG_SLAVE) && (!GETB(p_big->big_info, LLD_BIG_SYNC_ESTAB) && (BLE_UTIL_EVT_CNT_DIFF(p_big->big_evt_cnt, p_big->first_big_evt_cnt) >= 5)))
    {
    ...
    // Check if at least one channel has to be stopped
    if (stop)
    {
        // Free the BIG structure
        lld_big_free(p_big, true, false);
    ...
    else
    {
        ...
        bool was_prog = GETB(p_big->big_info, LLD_BIG_PROG);
        ...
        while (p_bis)
        {
            // Perform operation for end of channel event
            lld_bis_end_evt(p_bis, p_big);
...
__STATIC void lld_bi_end_sched_act(struct lld_bi_se *p_ref_se, struct lld_big_env *p_big)
{
    ...
    if (GETB(p_big->big_info, LLD_BIG_SLAVE) && GETB(p_big->big_info, LLD_BIG_SYNC))
    {
        ...
        int32_t big_anchor_bit_off = p_big->anchor_bit_off + p_big->slave_offset;
    ...
    do
    {
        ...
        p_ref_se = &(p_big->p_se_infos[p_big->ref_se_idx]);
        clock    = lld_read_clock();

        // next program target
        prog_tgt = CLK_ADD_2(p_big->anchor_ts, p_ref_se->offset / HALF_SLOT_SIZE);

        #if (BLE_OBSERVER)
        // If device is receiver, get the timestamp at the beginning of the sync window
        if(GETB(p_big->big_info, LLD_BIG_SLAVE))
        {
            prog_tgt = CLK_SUB(prog_tgt, CO_DIVIDE_ROUND((p_big->rx_win_size >> 1), HALF_SLOT_SIZE));
        }
        #endif // (BLE_OBSERVER)

        // schedule immediately for master or if it must be immediately done by slave.
        if (   !GETB(p_big->big_info, LLD_BIG_SLAVE)
            || CLK_LOWER_EQ(prog_tgt, CLK_ADD_2(clock, LLD_BI_ALARM_MIN_DISTANCE)))
        {
        ...
        else
        {
            ...
            p_alarm->time.hs = CLK_SUB(prog_tgt, LLD_BI_ALARM_DISTANCE);
            p_alarm->time.hus = 0;
...
__STATIC bool lld_bi_end_subevt(struct lld_bis_env* p_bis, struct lld_big_env *p_big, uint8_t se_status)
{
    ...
    if (p_bis->chan_idx != 0)
    {
        ...
        // Check if slave reception succeed.
        #if (BLE_OBSERVER)
        if (GETB(p_big->big_info, LLD_BIG_SLAVE) && !GETB(p_data->data_info, LLD_BIS_DATA_DONE) && GETB(se_status, LLD_BI_SE_STATUS_RX_OK))
        {
...
/**
 ****************************************************************************************
 * @brief Maintain channel assessment information
 ****************************************************************************************
 */
__STATIC void lld_bis_chan_assess(bool rx_ok)
{
    ...
    // Determine if reception has been corrupted or was correct
    if (rx_ok)
    {
        lld_channel_assess(usedchidx, true, clock);
    }
    else if (rssi > rwip_rf.rssi_interf_thr)
    {
        lld_channel_assess(usedchidx, false, clock);
    }
}
...
/**
 ****************************************************************************************
 * @brief Check the reception during channel activity
 *
 ...
 *
 * @return true if data packet has been properly received, else false
 ****************************************************************************************
 */
__STATIC bool lld_bis_rx(struct lld_bis_env *p_bis, struct lld_big_env *p_big, uint32_t timestamp)
{
    // Reception status
    bool rx_ok = false;
    ...
    if (lld_rxdesc_check(EM_BLE_CS_ACT_ID_TO_INDEX(p_bis->act_id)))
    {
        ...
        do
        {
            ...
            else
            {
                ...
                p_big->slave_offset = (CLK_DIFF(p_se->exp_sync_ts, base_cnt) * HALF_SLOT_SIZE)
            ...
            {
                // Keep in mind that a MIC error has been detected, channel will be stopped
                // as soon as possible
 ...
 ****************************************************************************************
 */
__STATIC void lld_bis_scan_rx(struct lld_bis_env *p_bis, struct lld_big_env *p_big)
{
    // Check if a descriptor has been used
    if (lld_rxdesc_check(EM_BLE_CS_ACT_ID_TO_INDEX(p_bis->act_id)))
    {
        // Maintain channel assessment information
        lld_bis_chan_assess(false);
...
__STATIC void lld_bis_skip_subevent(struct lld_bi_se *p_se, bool prog)
{
    ...
    uint8_t cs_idx = EM_BLE_CS_ACT_ID_TO_INDEX(p_se->chan_act_id);
...
__STATIC void lld_bi_evt_start_cbk(struct sch_arb_elt_tag *p_elt)
{
    ...
    if (p_elt != NULL)
    {
        ...
        // New event of a BIG
        if (GETB(p_big->big_info, LLD_BIG_NEW_EVT))
        {
            uint8_t hop_tog = GETB(p_big->big_info, LLD_BIG_HOP_TOG);

            // Update hopping sequence to use
            struct lld_bis_env * p_bis = (struct lld_bis_env*) co_list_pick(&(p_big->list_bis));
            while(p_bis != NULL)
            {
                if(p_bis->chan_idx != 0)
                {
                    em_ble_hopptr_hop_seq_ptr_setf(EM_BLE_CS_ACT_ID_TO_INDEX(p_bis->act_id), p_bis->hop_ptr[hop_tog] >> 2);
                }
                p_bis = (struct lld_bis_env*) p_bis->hdr.next;
            }
            ...
            // Store new value of hopping toggle bit
            SETB(p_big->big_info, LLD_BIG_HOP_TOG, hop_tog);

            lld_bi_compute_hop_scheme(p_big, p_big->big_evt_cnt + 1);

            // Mark that new BIG event start is over
            SETB(p_big->big_info, LLD_BIG_NEW_EVT, 0);
        }
    }
...
__STATIC void lld_bi_evt_canceled_cbk(struct sch_arb_elt_tag *p_elt)
{
    ...
    if (p_elt != NULL)
    {
        ...
        // Increment priority
        p_elt->current_prio += RWIP_PRIO_INC(p_big->prio_idx);
                             ...
                             uint32_t timestamp, uint8_t type, bool abort)
{
    ...
    #if (BLE_OBSERVER)
    if (GETB(p_big->big_info, LLD_BIG_SLAVE) && (type != LLD_BI_DUMMY_TYPE_CHAN_SCAN))
    {
        // Check reception status for this subevent
        if (lld_bis_rx(p_bis, p_big, timestamp))
        {
    ...
    if (!lld_bi_end_subevt(p_bis, p_big, se_status))
    {
        ...
        for (cnt = p_big->in_proc_se_idx; cnt < max_se_idx ; cnt++)
        {
            ...
            lld_bi_end_subevt(LLD_BIS_GET(p_cur_se->chan_act_id), p_big, LLD_BI_SE_STATUS_REJ_BIT);
        }
    }
...
__STATIC void lld_bi_skip_isr(struct lld_bis_env *p_bis, struct lld_big_env *p_big, uint8_t sub_evt_idx)
{
    ...
    // Check if the skipped sub-event is first element of program queue
...
__STATIC void lld_bi_frm_cbk(uint32_t timestamp, uint32_t dummy, uint8_t irq_type)
{
    ...
    // Get BIS structure
    struct lld_bis_env* p_bis       = LLD_BIS_GET((uint8_t)GETF(dummy, LLD_BI_DUMMY_CHAN_HDL));
    // Get BIG structure
...
uint8_t lld_big_start(struct lld_big_params *p_big_params, struct big_info *p_big_info, const uint8_t* p_big_info_data)
{
    ...
    do
    {
        ...
        // Allocate a new BIG structure
        p_big = lld_big_alloc(p_big_params, p_big_info, p_big_params->nb_pt_evt);
        ...
        for (cnt = 0; cnt < p_big_info->num_bis + 1; cnt++)
        {
            ...
            struct lld_bis_params *p_bis_params = &p_big_params->chan_params[cnt];
            ...
            if(p_bis->chan_idx > 0)
            {
                ...
                lld_isoal_start(p_bis->act_id,
                                (p_big_params->role == ROLE_MASTER ? ISO_SEL_TX : ISO_SEL_RX),
                                p_bis->act_id,
            ...
            lld_bi_fill_cs(p_bis, p_big, p_big_params, p_big_info->base_crc_init, p_big_info->giv);

            #if (BLE_BROADCASTER)
            if ((p_bis_params->chan_idx == 0) && !GETB(p_big->big_info, LLD_BIG_SLAVE))
            {
        ...
        // ====================================================================
        if(p_big_params->role == ROLE_MASTER)
        {
            lld_big_init_info(p_big_params->chan_params[0].chan_act_id, p_big_info_data,
        ...
        if (   GETB(p_big->big_info, LLD_BIG_SLAVE)
            && CLK_GREATER_THAN(p_big->anchor_ts, CLK_ADD_2(clock, LLD_BI_ALARM_MIN_DISTANCE)))
        {
            // Get pointer to alarm
            struct sch_alarm_tag *p_alarm = &p_big->alarm;

            // Program alarm for compensation of drift
            p_alarm->time.hs = CLK_SUB(p_big->anchor_ts, LLD_BI_ALARM_DISTANCE);
            p_alarm->time.hus = 0;

            // Indicate BIG is under initialization and alarm timer is running
            SETB(p_big->big_info, LLD_BIG_ALARM, 1);
...
uint8_t lld_big_stop(uint8_t grp_hdl, uint8_t reason)
{
    ...
    // Check if indicated channel exists
    if (p_big != NULL)
    {
        // Set stop reason
        p_big->stop_reason = reason;

        // If BIG event is in progress do not stop the channel now
...
uint8_t lld_big_cntl_tx(uint8_t grp_hdl, uint16_t em_ptr, uint8_t length, uint8_t nb_tx)
{
    ...
    do
    {
        ...
        // Only master can send a Control PDU
        if(GETB(p_big->big_info, LLD_BIG_SLAVE))
        {
                            ...
                            uint8_t per_ext_hdr_len, bool init)
{
    ...
    if ((p_big != NULL) && !GETB(p_big->big_info, LLD_BIG_STOP))
    {
        ...
        // Compute if following anchor point and ensure that we have at least 1 slot between anchor_ts and next periodic adv packet
        ...
        {
            big_unit = 1;
            big_offset = big_offset / 300; // we must compute truncated value
        }
        else // Use 30us big offset step
        {
            big_unit = 0;
            big_offset = big_offset / 30; // we must compute truncated value
        }
        ...
        em_ptr -= per_ext_hdr_len + 2;

        // copy periodic ADV extended header for first transmission
        if(init)
        {
            em_cpy(*per_ext_hdr_em_ptr, em_ptr, per_ext_hdr_len);
        }

        // update the periodic extended header to use
        *per_ext_hdr_em_ptr = em_ptr;
    }
...
uint8_t lld_bis_stats_get(uint8_t act_id, uint32_t* crc_error_packets, uint32_t* rx_unreceived_packets)
{
    ...
    // Check that BIG is enabled
    if ((p_big != NULL) && !GETB(p_big->big_info, LLD_BIG_STOP) && GETB(p_big->big_info, LLD_BIG_SLAVE))
    {
        // Get index of used control structure
