...
struct lld_adv_env_tag
{
    ...
    /// Periodic advertising interval in slots (625 us)
...
__STATIC void lld_adv_sync_info_set(uint8_t act_id)
{
    ...
    lld_per_adv_sync_info_get(adv_par->per_adv_id, &sync_ind_ts, &pa_evt_cnt, &map);
    ...
    while (diff_us <= (int32_t) (adv_par->aux_adv_ind_dur_us + BLE_AFS_DUR))
    {
        sync_ind_ts = CLK_ADD_2(sync_ind_ts, adv_par->per_adv_intv*4);
        pa_evt_cnt++;
        diff_hs = CLK_DIFF(evt->time.hs, sync_ind_ts);
        diff_us = ((diff_hs*HALF_SLOT_SIZE) >> 1) - adv_par->prim_aux_offset*30;
    }
    ...
    map.map[4] = (map.map[4] & 0x1F) | (lld_local_sca_get() << 5);
...
__STATIC void lld_adv_aux_evt_start_cbk(struct sch_arb_elt_tag* evt)
{
    ...
    if(evt != NULL)
    {
        ...
        em_ble_txauxptr0_pack(txdesc_idx, /*txauxoffsetlsb*/ adv_par->prim_aux_offset, /*txauxoffsetunit*/ 0, /*txauxca*/ (lld_env.local_drift <= 50), /*txllch*/ adv_par->aux_ch_idx);
...
__STATIC void lld_adv_evt_canceled_cbk(struct sch_arb_elt_tag* evt)
{
    ...
    if(evt != NULL)
    {
        ...
        if (adv_par->properties & ADV_DIRECT_HI)
        {
            evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_ADV_HDC_IDX);
        }
        else if (CLK_SUB(evt->time.hs, adv_par->last_prio_upd_ts) >= 2*adv_par->intv)
        {
            evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_ADV_IDX);
...
__STATIC void lld_adv_aux_evt_canceled_cbk(struct sch_arb_elt_tag* evt)
{
    ...
    if(evt != NULL)
    {
        ...
        {
            // Increment priority
            evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_ADV_AUX_IDX);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (adv_par->skip_en && (aux_evt->time.hs == timestamp))
                    {
                        ...
                        {
                            aux_evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_ADV_AUX_IDX);
                        }
        ...
        if(lld_adv_env[act_id] != NULL)
        {
            ...
            else
            {
                ...
                switch (adv_par->properties)
                {
                    ...
                    {
                        txtype = BLE_ADV_IND;
                        // prim_adv_ch_nb*(adv_int + min_scan_dur + T_IFS) - T_IFS
                        evt->duration_min = adv_par->prim_adv_ch_nb*2*(BLE_ADV_INT_US + BLE_ADV_MIN_SCAN_WIN_SIZE_US + BLE_IFS_DUR) - 2*BLE_IFS_DUR + BLE_RESERVATION_TIME_MARGIN_HUS;
                    }
                    break;
                    ...
                    case ADV_NONCONN_IND:
                    {
                        ...
                        // prim_adv_ch_nb*adv_int - T_IFS
                        evt->duration_min = adv_par->prim_adv_ch_nb*2*BLE_ADV_INT_US - 2*BLE_IFS_DUR + BLE_RESERVATION_TIME_MARGIN_HUS;
                    }
                    break;
                    default:
                    {
            ...
            if (adv_par->properties & ADV_DIRECT_HI)
            {
                ...
                em_ble_cntl_pack(cs_idx, RWIP_COEX_GET(ADV, TXBSY), RWIP_COEX_GET(ADV, RXBSY), RWIP_COEX_GET(ADV, DNABORT), EM_BLE_CS_FMT_HDC_ADV);
            }
            else
            {
                if (adv_par->properties & ADV_LEGACY)
                {
                    ...
                    em_ble_cntl_pack(cs_idx, RWIP_COEX_GET(ADV, TXBSY), RWIP_COEX_GET(ADV, RXBSY), RWIP_COEX_GET(ADV, DNABORT), EM_BLE_CS_FMT_LDC_ADV);
                }
                else
                {
                    // Extended advertiser
