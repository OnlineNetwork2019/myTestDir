...
__STATIC struct lld_sync_env_tag* lld_sync_env;
    ...
    while (lld_rxdesc_check(scan_evt->cs_idx))
    {
        ...
        if(((rxstat_pkt & LLD_ADV_ERR_MASK) == 0) || ((rxstat_pkt & LLD_ADV_ERR_MASK) == EM_BLE_PRIV_ERR_BIT))
        {
                ...
                switch (scan_evt->aux_state)
                {
                    case RX_ADV_EXT_IND:
                    {
                        ...
                        if (rxaelength > 0)
                        {
                            ...
                            if (rxaeheader & EM_BLE_RXADI_BIT) // AdvDataInfo
                            {
                                ...
                                if (!adi_match && (lld_sync_env == NULL))
                                {
                                    // Collision detected. Discard incomplete advertising report.
                                    ind->data_status = ADV_EVT_DATA_STATUS_DISCARDED;
                                    break;
                            ...
                            if (rxaeheader & EM_BLE_RXAUXPTR_BIT) // AuxPtr
                            {
                                ...
                                /* we can use FOLLOWAUXPTR for indication of the HW follow auxptr (regardless of ADI is present
                                ...
                                {
                                    uint8_t aux_phy = (aux_data & BLE_AUX_PHY_MASK) >> BLE_AUX_PHY_LSB;
                                    ...
                                    if (phy_support && aux_av && lld_calc_aux_rx(&scan_par->aux_rx_out, rxdesc_idx, aux_data))
                                    {
                                        ...
                                        scan_evt->aux_offload = SW_CNTL_AUX_OFFLOAD;
                                    }
                                    else
                                    {
                                ...
                                else
                                {
                                    ...
                                    scan_evt->aux_offload = HW_CNTL_AUX_OFFLOAD;
                                }
                    ...
                    case RX_AUX_ADV_IND:
                    {
                        ...
                        if (rxaelength > 0)
                        {
                            ...
                            bool create_sync = (lld_sync_env != NULL);
                            ...
                            if (ADV_EVT_DATA_STATUS_DISCARDED != ind->data_status)
                            {
                                if (rxaeheader & EM_BLE_RXAUXPTR_BIT) // AuxPtr
                                {
                                    ...
                                    if (0 == em_ble_rxstatadv_followauxptr_getf(rxdesc_idx))
                                    {
                                        ...
                                        if (aux_av && lld_calc_aux_rx(&scan_par->aux_rx_out, rxdesc_idx, aux_data))
                                        {
                                            // Calculated AUX RX activity for Software reschedule
                                            scan_evt->aux_offload = SW_CNTL_AUX_OFFLOAD;
                                        }
                            ...
                            if (rxaeheader & EM_BLE_RXSYNC_BIT) // SyncInfo
                            {
                                ...
                                if (create_sync && lld_scan_sync_accept(scan_id, rxdesc_idx, ind->addr_type, ind->peer_id_addr, rx_adv_sid))
                                {
                                    ...
                                    if (lld_scan_sync_info_unpack(&req_sync->syncinfo, rxdataptr + i))
                                    {
                                        ...
                                        req_sync->act_id = lld_sync_env->act_id;
                                        ...
                                        req_sync->adv_addr_type = lld_sync_env->adv_addr_type;
                                        memcpy(&req_sync->adv_addr.addr[0], &lld_sync_env->adv_addr.addr[0], BD_ADDR_LEN);

                                        ke_msg_send(req_sync);

                                        // Free memory used for create sync and stop looking for syncinfo
                                        ke_free(lld_sync_env);
                                        lld_sync_env = NULL;
                                    }
                                    else
                                    {
                            ...
                            if (rxaeheader & EM_BLE_RXAUXPTR_BIT) // AuxPtr
                            {
                                ...
                                if (0 == em_ble_rxstatadv_followauxptr_getf(rxdesc_idx))
                                {
                                    ...
                                    if (aux_av &&
                                        ...
                                        (scan_evt->rx_chain_cnt_pkt < BLE_ADV_FRAG_NB_RX_MAX) &&
                                        #endif //(BLE_ADV_FRAG_NB_RX_MAX)
                                        lld_calc_aux_rx(&scan_par->aux_rx_out, rxdesc_idx, aux_data))
                                    {
                                        // Calculated AUX RX activity for Software reschedule
                                        scan_evt->aux_offload = SW_CNTL_AUX_OFFLOAD;
                                    }
            ...
            if (ADV_EVT_DATA_STATUS_DISCARDED != ind->data_status)
            {
                do
                {
                    // Discard if Privacy Error, except Directed Advertisements which are conditionally processed
                    ...
                    if ((ind->evt_type == ADV_DIRECT_IND) || (rxaeheader & EM_BLE_RXTGTA_BIT))
                    {
                        ...
                        if (!em_ble_rxstatadv_bdaddr_match_getf(rxdesc_idx) && ((ADDR_PUBLIC == ind->rx_rxadd) || (ind->target_id_addr.addr[BD_ADDR_LEN-1] & 0xC0) == RND_STATIC_ADDR))
                        {
                            ind->data_status = ADV_EVT_DATA_STATUS_DISCARDED;
                            break;
                        }
                    ...
                    if (addr_type != ADDR_NONE)
                    {
                        ...
                        // Determine if local RPA mismatch despite peer_add_match
                        ind->priv_err = ((rxstat_pkt & EM_BLE_PRIV_ERR_BIT) != 0);
                    }
...
__STATIC bool lld_scan_sync_accept(uint8_t scan_id, uint8_t index_pkt, uint8_t addr_type, struct bd_addr rx_adva, uint8_t rx_adv_sid)
{
    ...
    if (PER_SYNC_FILT_USE_PAL == lld_sync_env->filter_policy)
    {
        ...
        if ((scan_evt->peradv_filt_en) && em_ble_rxstatadv_in_peradval_getf(index_pkt))
        {
            ...
            // Store to lld_sync_env for device to use & validation with SID
            memcpy((void*)&lld_sync_env->adv_addr.addr[0], &rx_adva.addr[0], BD_ADDR_LEN);
            lld_sync_env->adv_addr_type = addr_type;
    ...
    else // PER_SYNC_FILT_IGNORE_PAL - Check explicit parameters
    {
        ...
        if ((lld_scan_env->ral_resol_en) && (0 != em_ble_rxralptr_getf(index_pkt)))
        {
            ...
            sync_accept = ((addr_type == lld_sync_env->adv_addr_type) && !memcmp(&peer_id_addr.addr[0], &lld_sync_env->adv_addr.addr[0], BD_ADDR_LEN)
                    && (lld_sync_env->adv_sid == rx_adv_sid));

            if (sync_accept)
            {
                // Address resolution enabled, and found in the RAL
                memcpy((void*)&lld_sync_env->adv_addr.addr[0], &rx_adva.addr[0], BD_ADDR_LEN);
                lld_sync_env->adv_addr_type = ADDR_RAND;
            }
        }
        else
        {
            // Explicitly check Type, AdvA and SID with Create Sync Parameters.
            sync_accept = ((addr_type == lld_sync_env->adv_addr_type) && !memcmp(&rx_adva.addr[0], &lld_sync_env->adv_addr.addr[0], BD_ADDR_LEN)
                    && (lld_sync_env->adv_sid == rx_adv_sid));
        }
    }
...
{
    DBG_SWDIAG(LESCAN, FRM_RX_ISR, 1);
    ...
    DBG_SWDIAG(LESCAN, FRM_RX_ISR, 0);
}
...
__STATIC void lld_scan_frm_skip_isr(uint8_t scan_id)
{
    ...
    if((lld_scan_env != NULL) && (lld_scan_env->params[phy_idx] != NULL))
    {
        ...
        else
        {
            ...
            {
                // Increment priority
                evt->current_prio += RWIP_PRIO_INC(RWIP_PRIO_SCAN_IDX);
...
__STATIC void lld_scan_evt_start_cbk(struct sch_arb_elt_tag* evt)
{
    ...
    if(evt != NULL)
    {
        ...
        scan_evt->peradv_filt_en = ((lld_sync_env) && (0 != lld_sync_env->filter_policy));
...
uint8_t lld_scan_start(uint8_t act_id, struct lld_scan_params* params)
{
    ...
    if(lld_scan_env == NULL)
    {
        ...
        lld_scan_env = LLD_ALLOC_EVT(lld_scan_env_tag);

        if(lld_scan_env != NULL)
        {
            // Point to parameters
            scan_env = lld_scan_env;
            ...
            LLD_INIT_EVT(lld_scan_env, lld_scan_env_tag);
            ...
            for (uint8_t phy_idx=0; phy_idx < MAX_SCAN_PHYS; phy_idx++)
            {
                ...
                for (uint8_t evt_idx=0; evt_idx < NUM_SCAN_EVTS; evt_idx++)
                {
                     ...
                     */
                    scan_par->evt[SCAN_EVT_DFT].peradv_filt_en = ((lld_sync_env) && (0 != lld_sync_env->filter_policy));
                    ...
                    em_ble_cntl_pack(cs_idx, RWIP_COEX_GET(SCAN, TXBSY), RWIP_COEX_GET(SCAN, RXBSY), RWIP_COEX_GET(SCAN, DNABORT), cs_fmt);
...
{
    // Check if create sync is active
    ASSERT_ERR(lld_sync_env == NULL);

    GLOBAL_INT_DISABLE();

    lld_sync_env = LLD_ALLOC_EVT(lld_sync_env_tag);

    lld_sync_env->act_id = act_id;

    lld_sync_env->filter_policy = filter_policy;

    if(adv_addr != NULL)
    {
        lld_sync_env->adv_sid = adv_sid;
        lld_sync_env->adv_addr_type = adv_addr_type;
        memcpy(&lld_sync_env->adv_addr.addr[0], &adv_addr->addr[0], BD_ADDR_LEN);
    }
