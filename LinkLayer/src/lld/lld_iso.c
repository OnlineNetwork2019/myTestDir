...
void lld_iso_init(uint8_t init_type)
{
    switch (init_type)
    {
        ...
        case RWIP_RST:
        {
            // flush hopping request that wait to be executed
            while (!co_list_is_empty(&lld_iso_env.iso_hop_list))
            {
                struct lld_iso_hop_inf* hop = (struct lld_iso_hop_inf*) co_list_pop_front(&lld_iso_env.iso_hop_list);
                ke_free(hop);
            }
        } break;
        ...
        case RWIP_1ST_RST:
        {
            ...
            // by default set stream and channel pointers to null
            memset(&(lld_iso_env.p_groups), 0, sizeof(lld_iso_env.p_groups));
            memset(&(lld_iso_env.p_channels), 0, sizeof(lld_iso_env.p_channels));
        }
        break;
