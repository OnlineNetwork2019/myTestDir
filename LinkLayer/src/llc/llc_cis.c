...
__STATIC void llc_cis_ind_ack(uint8_t link_id, uint8_t op_code)
{
    ...
    if (state == LLC_LOC_CIS_WAIT_IND_ACK_OR_EST)
    {
        ...
        lld_cis_start_sup_to(p_create_ind->chan_hdl);
 ...
 *                          parameters
 * @param[in] grp_offset    Offset in group (in us)
 ****************************************************************************************
 */
__STATIC void llc_cis_ind_pdu_send(uint8_t link_id, struct llc_op_cis_create_ind *p_create_ind, uint32_t grp_offset)
{
    // Get PDU structure
    ...
    pdu.conn_event_cnt      = p_create_ind->conn_event_cnt;
    pdu.cis_sync_delay      = p_create_ind->air_time - grp_offset;
 ...
 *
 * @param[in] link_id       Identifier of link on which PDU has to be sent
 * @param[in] p_create_ind  Pointer to the procedure structure containing the negociated
 *                          parameters
 ****************************************************************************************
 */
...
__STATIC void llc_cis_loc_create_proc_continue(uint8_t link_id, uint8_t state, uint8_t status)
{
    ...
    else
    {
        ...
        // load CIS parameters
        lli_ci_param_get(p_create_ind->chan_hdl, &p_cig_param, &p_cis_param);
        ...
        switch (llc_proc_state_get(&p_create_ind->proc))
        {
            case LLC_LOC_CIS_CREATE_START:
            {
                ...
                p_create_ind->conn_event_cnt = lld_con_event_counter_get(link_id) + LLC_PROC_CIS_INSTANT_DELAY;
                ...
                dist_con_evt_to_iso = CO_MOD(CLK_SUB(con_evt_time_hs, p_create_ind->act_offset), (p_cig_param->iso_interval<<2));
                if(dist_con_evt_to_iso > 0)
                {
                    dist_con_evt_to_iso = (p_cig_param->iso_interval<<2) - dist_con_evt_to_iso;
                }

                // Convert distance to us
                dist_con_evt_to_iso = ((dist_con_evt_to_iso * HALF_SLOT_SIZE + 1) >> 1);

                // Compute distance from connection event to ISO event (in us)
                p_create_ind->cis_offset_min = (uint32_t)dist_con_evt_to_iso + (p_create_ind->act_offset_hus>>1) + p_cis_param->grp_offset;
            ...
            case LLC_LOC_CIS_WAIT_CREATE_RSP:
            {
                ...
                iso_evt_time_hus = con_evt_time_hus + 2*p_create_ind->cis_offset_min - 2*p_cis_param->grp_offset;
                ...
                // Convert offset in us
                dist_con_evt_to_iso = ((dist_con_evt_to_iso * HALF_SLOT_SIZE + 1) >> 1);

                // add the internal CIS offset
                p_create_ind->cis_offset = dist_con_evt_to_iso + p_cis_param->grp_offset + (p_create_ind->act_offset_hus >> 1);

                // Generate access address of CIS channel to create
                lld_cis_aa_gen(link_id, &p_create_ind->access_addr);

                // Start the CIS
                status = lli_cis_start(p_create_ind->chan_hdl, p_create_ind->access_addr,
                ...
                if(status == CO_ERROR_NO_ERROR)
                {
                    ...
                    llc_cis_ind_pdu_send(link_id, p_create_ind, p_cis_param->grp_offset);
    ...
    if (finished)
    {
        ...
        if ((status != CO_ERROR_NO_ERROR) && (llc_proc_state_get(&p_create_ind->proc) >= LLC_LOC_CIS_WAIT_IND_ACK_OR_EST))
        {
            ...
            lli_cis_stop(p_create_ind->chan_hdl, CO_ERROR_CON_TERM_BY_LOCAL_HOST);
        }

        // Inform LLI CIS module that creation procedure is finished
        lli_cis_create_nego_end(p_create_ind->chan_hdl, status, true);
...
__STATIC void llc_cis_rem_create_proc_continue(uint8_t link_id, uint8_t state, uint8_t status)
{
    ...
    else
    {
        ...
        switch (llc_proc_state_get(&p_create_ind->proc))
        {
            ...
            case LLC_REM_CIS_WAIT_HOST_STATUS:
            {
                ...
                // load CIS parameters
                lli_ci_param_get(p_create_ind->chan_hdl, &p_cig_param, NULL);
                ...
                if(((act_offset_hus == 0) + ((act_offset_hus + act_margin_hus) / HALF_SLOT_SIZE)) > 1)
                {
                    ...
                    req_param.conhdl          = BLE_CHANHDL_TO_CISHDL(p_create_ind->chan_hdl);
                ...
                // Convert distance to hus
                dist_con_evt_to_iso = (dist_con_evt_to_iso * HALF_SLOT_SIZE + 1);
    ...
    if (finished)
    {
        ...
        lli_cis_create_nego_end(p_create_ind->chan_hdl, status, (state != LLC_REM_CIS_WAIT_HOST_STATUS));
    ...
    else
    {
        ...
        switch (llc_proc_state_get(&p_stop_ind->proc))
        {
            case LLC_LOC_CIS_STOP_START:
            {
                ...
                // CIS channel parameters
                lli_ci_param_get(p_stop_ind->chan_hdl, NULL, &p_cis_param);
            ...
            {
                // Inform LLI CIS module that driver can be stopped
                lli_cis_stop(p_stop_ind->chan_hdl, CO_ERROR_CON_TERM_BY_LOCAL_HOST);
...
uint8_t ll_cis_req_handler(uint8_t link_id, struct ll_cis_req *p_pdu, uint16_t event_cnt)
{
    ...
    do
    {
        ...
        if (   (p_pdu->cig_id > BLE_CIG_MAX_ID)                     || (p_pdu->cis_id > BLE_CIS_MAX_ID)
            ...
            || (p_pdu->sdu_interval_m2s < BLE_ISO_MIN_SDU_INTERVAL) || (p_pdu->sdu_interval_s2m < BLE_ISO_MIN_SDU_INTERVAL)
            || (p_pdu->sdu_interval_m2s > BLE_ISO_MAX_SDU_INTERVAL) || (p_pdu->sdu_interval_s2m > BLE_ISO_MAX_SDU_INTERVAL)
        ...
        if(framing == ISO_UNFRAMED_MODE)
        {
            ...
            if(   (CO_MOD(iso_intv_us, p_pdu->sdu_interval_m2s) != 0)
               || (CO_MOD(iso_intv_us, p_pdu->sdu_interval_s2m) != 0))
            {
            ...
            if (   (CO_MOD(GETF(p_pdu->bn, LLCP_CIS_BN_M2S), iso_intv_us / p_pdu->sdu_interval_m2s) != 0)
                || (CO_MOD(GETF(p_pdu->bn, LLCP_CIS_BN_S2M), iso_intv_us / p_pdu->sdu_interval_s2m) != 0)   )
            {
                status = CO_ERROR_UNSUPPORTED;
                break;
            }
        ...
        status = lli_cis_create_req(link_id, p_pdu, framing, &chan_hdl);
        ...
        if (status == CO_ERROR_NO_ERROR)
        {
            ...
            p_create_ind->chan_hdl         = chan_hdl;
...
uint8_t ll_cis_ind_handler(uint8_t link_id, struct ll_cis_ind *p_pdu, uint16_t event_cnt)
{
    ...
    do
    {
        ...
        else
        {
            ...
            p_create_ind->conn_event_cnt  = p_pdu->conn_event_cnt;
            p_cig_param->grp_air_time     = p_pdu->cig_sync_delay;
            p_cis_param->grp_offset       = p_pdu->cig_sync_delay - p_pdu->cis_sync_delay;
...
uint8_t ll_cis_rsp_handler(uint8_t link_id, struct ll_cis_rsp *p_pdu, uint16_t event_cnt)
{
    ...
    do
    {
        ...
        if (   (p_pdu->cis_offset_min < BLE_CIS_MIN_OFFSET) || (p_pdu->cis_offset_min > p_pdu->cis_offset_max)
            || (p_pdu->cis_offset_max > ((p_llc_env->con_params.interval*2*SLOT_SIZE) - p_cis_param->sub_evt_dur)))
        {
            status = CO_ERROR_INVALID_LMP_PARAM;
            break;
        }
...
{
    // Get channel index
    uint8_t chan_hdl = lli_cis_chan_hdl_get(link_id, p_pdu->cig_id, p_pdu->cis_id);

    // Check that indicated channel is valid
    if (chan_hdl != BLE_INVALID_CHANHDL)
    {
        // Inform the LLI CIS module than the CIS driver can be stopped
        lli_cis_stop(chan_hdl, p_pdu->err_code);
    }
...
uint8_t llc_cis_create_req(uint8_t link_id, uint8_t chan_hdl, uint16_t act_offset, uint16_t act_margin)
{
    ...
    do
    {
        ...
        lli_ci_param_get(chan_hdl, &p_cig_param, &p_cis_param);
        ...
        p_create_ind->act_offset_hus  = 0;
        p_create_ind->air_time        = p_cig_param->grp_air_time;
