...
int ll_cte_req_handler(uint8_t link_id, struct ll_cte_req *pdu, uint16_t event_cnt)
{
    ...
    else if(!llc_env_ptr->cte_tx.en || !(llc_env_ptr->cte_tx.cte_types & (1 << cte_type)))
    {
        llc_ll_reject_ind_pdu_send(link_id, LL_CTE_REQ_OPCODE, CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE, NULL);
    }
...
int hci_le_con_cte_rsp_en_cmd_handler(uint8_t link_id, struct hci_le_con_cte_rsp_en_cmd const *param, uint16_t opcode)
{
    ...
    do
    {
        ...
        // If the Host issues this command before issuing the HCI_LE_Set_Connection_CTE_Receive_Parameters command 
        //at least once on the connection
        if(!llc_env_ptr->cte_tx.param_received)
            break;

        // If the Host issues this command when the transmitter PHY for the connection is not a PHY 
        //that allows Constant Tone Extensions
        ...
        if(param->en == 1)
        {
            ...
            llc_env_ptr->cte_tx.en = true;
        }
        else
        {
            // Disable CTE responses
            llc_env_ptr->cte_tx.en = false;
        }
