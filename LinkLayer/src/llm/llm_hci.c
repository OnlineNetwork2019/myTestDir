...
__STATIC uint8_t const llm_local_supp_cmds[46] =
{
        BLE_CMDS_BYTE0,
        0x00,
        BLE_CMDS_BYTE2,
...
__STATIC int hci_host_nb_cmp_pkts_cmd_handler(struct hci_host_nb_cmp_pkts_cmd const *param, uint16_t opcode)
{
    ...
    {
        // BLE ACL link
        if (param->con_hdl[idx] < BLE_ACTIVITY_MAX)
        {
...
__STATIC int hci_le_add_dev_to_rslv_list_cmd_handler(struct hci_le_add_dev_to_rslv_list_cmd const *param, uint16_t opcode)
{
    ...
    do
    {
        ...
        {
            if((llm_env.act_info[i].state == LLM_CONNECTED) && !memcmp(&param->peer_id_addr.addr[0], &llm_env.act_info[i].info.con.bd_addr.addr[0], BD_ADDR_LEN))
            {
                connected = true;
                break;
            }
        ...
        lld_res_list_add(position, &param->peer_id_addr, &param->peer_irk, &param->local_irk, param->peer_id_addr_type, connected, in_wl);
...
__STATIC int hci_le_rd_local_p256_public_key_cmd_handler(void const *param, uint16_t opcode)
{
    ...
    if(!forced_key)
    {
        ...
        ecc_gen_new_secret_key(secret_key256, false);
    }

    // Generate the associated public key
    ecc_gen_new_public_key(secret_key256, LLM_PUB_KEY_GEN_IND, TASK_LLM);
...
__STATIC int hci_le_gen_dhkey_v1_cmd_handler(struct hci_le_gen_dhkey_v1_cmd *param, uint16_t opcode)
{
    ...
    for(i = PRIV_KEY_256_LEN-1 ; i > 0 ; i--)
    {
    ...
    {
        // Generate DH key
        status = ecc_generate_key256(ECC_DHKEY_GENERATION, p_secret_key, &param->public_key[0], &param->public_key[32], LLM_DH_KEY_GEN_IND, TASK_LLM, false);
    }
...
__STATIC int hci_le_gen_dhkey_v2_cmd_handler(struct hci_le_gen_dhkey_v2_cmd *param, uint16_t opcode)
{
    ...
    if(param-> key_type == USE_GEN_PRIV_KEY)
    {
        // Check if private key exist
...
//__STATIC const struct llm_hci_cmd_handler llm_hci_command_handler_tab[] =
 ...
 */
/* 
int hci_command_llm_handler(ke_msg_id_t const msgid, 
							void const *param, 
...
KE_MSG_HANDLER_NO_STATIC(hci_command_llm, void)
{
    ...
    for(int i = 0; i < ARRAY_LEN(llm_hci_command_handler_tab); i++)
    {
