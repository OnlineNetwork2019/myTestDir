...
uint8_t llm_per_adv_chain_dur(uint16_t len, uint8_t phy)
{
   ...
   uint32_t dur_hus = pkt_nb*(2*ble_util_pkt_dur_in_us(frag_size, phy - 1) + BLE_AFS_DUR);
...
void llm_ch_map_update(void)
{
    ...
    struct lld_ch_assess_data* ch_assess = lld_ch_assess_data_get();
    ...
    for(int i = 0 ; i < DATA_CHANNEL_NB ; i++)
    {
        ...
        if(llm_env.ch_ass_en)
        {
            // Channel assessment is considered only if a packet has been received recently
            if(CLK_SUB(clock, ch_assess->timestamp[i]) < BLE_CH_ASSESS_VALID_TO)
            {
                // Controller assesses the channel as bad
                if (ch_assess->level[i] <= BLE_CH_ASSESS_COUNT_THR_BAD)
                {
                    ch_map->map[byte_idx] &= ~(1 << bit_pos);
                    continue;
                }
            ...
            else
            {
                ...
                if(CLK_SUB(clock, ch_assess->timestamp[i]) > BLE_CH_REASSESS_TO)
                {
                    ch_map->map[byte_idx] |= (1 << bit_pos);
                    continue;
                }
    ...
    if(nbchgood < DATA_CHANNEL_USED_NB_MIN)
    {
        ...
        for(int i = 0 ; i < DATA_CHANNEL_NB ; i++)
        {
            ...
            if (((host_ch_class->map[byte_idx] >> bit_pos) & 0x1) == 0x01)
            {
                ch_map->map[byte_idx] |= (1 << bit_pos);
    ...
    // Search for master active link(s) or periodic advertising
    ...
    switch (init_type)
    {
        ...
        case RWIP_1ST_RST:
        {
            ...
            // Channel assessment
            {
                ...
                if (rwip_param.get(PARAM_ID_CH_ASS_EN, &length, &ch_ass_en) != PARAM_OK)
                {
                    // If no value is set in persistent storage enable the feature by default
                    llm_env.ch_ass_en = true;
                }
                else
                {
                    // Bit 0 denotes support for BLE
                    llm_env.ch_ass_en = GETB(ch_ass_en, CH_ASS_EN_BLE);
                }
            }
...
void llm_link_disc(uint8_t link_id)
{
    ...
    position = llm_ral_search(&llm_env.act_info[link_id].info.con.bd_addr, llm_env.act_info[link_id].info.con.addr_type);
    if(position < BLE_RAL_MAX)
    {
        lld_res_list_peer_update(position, false);
    }
