...
extern int hci_vs_set_iso_data_path_trigger_cmd_handler(struct hci_vs_set_iso_data_path_trigger_cmd const *param, uint16_t opcode);
...
HCI_CMD_HANDLER_TAB(lli)
{
    ...
    #endif // (BLE_BIS || BLE_CIS)
    {HCI_VS_SET_ISO_DATA_PATH_TRIGGER_CMD_OPCODE,   (lli_hci_cmd_hdl_func_t) hci_vs_set_iso_data_path_trigger_cmd_handler  },
...
KE_MSG_HANDLER(hci_command_lli, void)
{
    ...
    for (int i = 0; i < ARRAY_LEN(lli_hci_command_handler_tab); i++)
    {
