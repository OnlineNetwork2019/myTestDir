...
/// Number of packet transmission attemtps
#define LLI_BIS_NB_TX_ATTEMPT   6

/// Retrieve BIS environment
#define LLI_BIS_ENV_GET(channel, env_ptr)\
    lli_chan_env_get(channel, LLI_ISO_CHAN_BIS, (struct lli_chan_env**) (env_ptr))
...
struct lli_big_env
{
    ...
    uint8_t                      chan_hdl[__ARRAY_EMPTY];
};
                                      ...
                                      uint8_t rate, uint32_t trans_latency)
{
    ...
    if(status == CO_ERROR_NO_ERROR)
    {
        ...
        {
            uint8_t stream_hdl = p_big_env->chan_hdl[cursor];

            // fill BIS connection handle
            p_evt->conhdl[cursor] =  BLE_CHANHDL_TO_BISHDL(stream_hdl);
        }
    }
    else
    {
...
/**
 ****************************************************************************************
 * @brief Remove all the BIG environment data
 *
 * @param[in] p_big_env Pointer to the BIG to remove
 ****************************************************************************************
 */
__STATIC void lli_big_remove(struct lli_big_env *p_big_env)
{
    uint8_t cursor;

    // mark stream disabled
    ...
    for(cursor = 0 ; cursor < p_big_env->nb_bis ; cursor++)
    {
        ...
        if(LLI_BIS_ENV_GET(stream_hdl, &p_bis_env) == CO_ERROR_NO_ERROR)
        {
            // Check if data path isn't disabled
            if(!data_path_is_disabled(p_bis_env->p_dp))
            {
                // Remove data path in use
                lli_data_path_remove(stream_hdl, (GETB(p_big_env->info, LLI_BIG_BROADCASTER) ? ISO_SEL_TX : ISO_SEL_RX),
                                     &(p_bis_env->p_dp));
            }

            // Clean-up BIS
            llm_activity_free_set(stream_hdl);
            lli_chan_cleanup(stream_hdl);
        }
        else
        {
            ASSERT_INFO(0, p_big_env->big_hdl, stream_hdl);
        }
    }

    // Clean-up environement group
 ...
 * @param[in]  role         Role of the channel (master or slave)
 ...
 ****************************************************************************************
 */
__STATIC uint8_t lli_bis_create(struct lli_bis_env **pp_bis_env, uint8_t role, uint8_t grp_hdl, uint8_t bis_id)
{
    struct lli_chan_env *p_bis_env;
    uint8_t  act_id = BLE_ACTIVITY_MAX;
    // get available activity
    uint8_t status;
    ...
    do
    {
        ...
        status = lli_chan_create(act_id, LLI_ISO_CHAN_BIS, sizeof(struct lli_bis_env), &p_bis_env);
        ...
        *pp_bis_env = (struct lli_bis_env*) p_bis_env;

        // Reserve an activity for the BIS
        llm_activity_bis_reserve(p_bis_env->hdl);

        // Initialize default channel parameters
         (*pp_bis_env)->grp_hdl = grp_hdl;
         (*pp_bis_env)->bis_id  = bis_id;

        lli_data_path_init(p_bis_env->hdl, (role == ROLE_MASTER) ? ISO_SEL_TX : ISO_SEL_RX, &((*pp_bis_env)->p_dp));
        (*pp_bis_env)->trigger_offset = LLI_INVALID_TRIGGER_OFFSET;
    } while (0);
...
__STATIC void lli_big_start(struct lli_big_env *p_big_env)
{
    ...
    if(p_big_params->role == ROLE_MASTER)
    {
        ...
        lli_big_saa_gen(p_big_params->chan_params[0].chan_act_id, &p_big_info->seed_access_addr);
    ...
    #endif // (BLE_BROADCASTER)
    #if (BLE_OBSERVER)
    if ((p_big_params->role == ROLE_SLAVE) && (p_big_env->max_se != 0))
    {
    ...
    // Start the channel
    status = lld_big_start(p_big_params, p_big_info, p_big_info_data);
    ...
    if(status != CO_ERROR_NO_ERROR)
    {
        // If an error occurs, perform a clean-up of BIG.
        lli_big_remove(p_big_env);
    }
}
...
__STATIC void lli_big_saa_gen(uint32_t activity_id, uint32_t *p_seed_acc_addr)
{
    *p_seed_acc_addr = co_rand_word();
    ...
    // SAA15-0 shall differ in at least two bits.
    *p_seed_acc_addr = (*p_seed_acc_addr & 0xFFFF0FF0) | ((activity_id & 0xF) << 0)| ((activity_id & 0xF) << 24);
 ...
 ****************************************************************************************
 */
__STATIC void lli_big_compute_params(const struct hci_le_create_big_cmd *p_param, struct hci_le_create_big_test_cmd *p_comp)
{
    ...
    p_comp->phy                   = p_param->phy;
    p_comp->packing               = p_param->packing;
    p_comp->irc                   = p_param->rtn + 1;
    p_comp->pto                   = 0;
...
uint8_t lli_bis_data_path_set(struct lli_chan_env *p_env, uint8_t direction, uint8_t data_path_type)
{
    ...
    struct lli_bis_env*   p_bis_env   = (struct lli_bis_env*) p_env;
    struct lli_big_env* p_big_env = NULL;

    do
    {
        uint8_t exp_direction;
        const struct data_path_itf** pp_dp = &(p_bis_env->p_dp);
        ...
        if ((status == CO_ERROR_NO_ERROR) && (GETF(p_big_env->info, LLI_BIG_STATE) == LLI_BIG_ENABLED))
        {
            // Update the data path to use
            status = lld_isoal_datapath_set(p_bis_env->chan.hdl, direction, *pp_dp,
                                            p_bis_env->trigger_offset != LLI_INVALID_TRIGGER_OFFSET,
                                            p_bis_env->trigger_offset);
        }
    } while (0);
...
uint8_t lli_bis_data_path_remove(struct lli_chan_env *p_env, uint8_t direction)
{
    ...
    if(status == CO_ERROR_NO_ERROR)
    {
        uint8_t exp_direction = GETB(p_big_env->info, LLI_BIG_BROADCASTER) ? ISO_SEL_TX : ISO_SEL_RX;

        // check that datapath is removed onto an expected stream direction
        if(exp_direction == direction)
        {
            // Check if data path isn't disabled
            if(!data_path_is_disabled(p_bis_env->p_dp))
            {
                ...
                {
                    // Remove data path in use
                    lld_isoal_datapath_remove(p_bis_env->chan.hdl, direction);
                }

                // remove it.
                lli_data_path_remove(p_env->hdl, direction, &(p_bis_env->p_dp));
            }
...
uint8_t lli_bis_trigger_set(struct lli_chan_env *p_env, uint8_t direction, bool enable, int32_t offset)
{
    ...
    do
    {
        ...
        if(GETF(p_big_env->info, LLI_BIG_STATE) == LLI_BIG_ENABLED)
        {
            ...
            status = lld_isoal_trigger_set(p_bis_env->chan.hdl, direction, enable, offset);
        }
        else
        {
            status = CO_ERROR_COMMAND_DISALLOWED;
...
int hci_le_create_big_test_cmd_handler(struct hci_le_create_big_test_cmd *p_param, uint16_t opcode)
{
    ...
    do
    {
        ...
        // check if all channels can be allocated
        if(lli_nb_chan_ava_get() < p_param->num_bis)
        {
        ...
        {
            uint16_t max_sdu = (  (p_param->max_pdu * p_param->bn * p_param->sdu_interval)
                                - (BLE_ISOAL_SEG_HEADER_SIZE * p_param->bn)) / iso_interval_us
        ...
        else
        {
            ...
            // add the margin
            sub_evt_dur = air_evt_dur + lli_sub_evt_margin_get();
            ...
            if(p_big_env == NULL)
            {
                ...
                // allocate a channel environment for all channels of the BIG
                for(i = 0 ; i < p_param->num_bis ; i++)
                {
                    status = lli_bis_create(&p_bis_env, ROLE_MASTER, p_big_env->group.hdl, i+1);
                    ...
                    // keep channel handle
                    p_big_env->chan_hdl[i] = p_bis_env->chan.hdl;
                    p_bis_env->chan.framing = p_param->framing;
                }
                ...
                if(status != CO_ERROR_NO_ERROR)
                {
                    ...
                    {
                        llm_activity_free_set(p_big_env->chan_hdl[i]);
                        lli_chan_cleanup(p_big_env->chan_hdl[i]);
                    }

                    // Set activity in reserved state and clean-up Goup activity
                    llm_activity_free_set(p_big_env->act_id);
                    lli_big_cleanup(p_big_env->group.hdl);
                }
                else
                {
                    ...
                    req_param.conhdl          = BLE_CHANHDL_TO_BISHDL(p_big_env->act_id);
                    ...
                    p_big_env->p_start = (struct lli_bi_start_params*)
                            ke_malloc(sizeof(struct lli_bi_start_params) + BLE_EXT_ACAD_BIG_INFO_ENC_LEN,
                    ...
                    p_big_params->chan_params[0].chan_act_id = p_big_env->act_id;
                    p_big_params->chan_params[0].chan_idx    = 0;

                    // Configure all other channels
                    for(i = 0 ; i < p_big_env->nb_bis ; i++)
                    {
                        p_big_params->chan_params[i+1].chan_act_id = p_big_env->chan_hdl[i];
                        p_big_params->chan_params[i+1].chan_idx    = i+1;
                    }
...
int hci_le_create_big_cmd_handler(struct hci_le_create_big_cmd *p_param, uint16_t opcode)
{
    ...
    // TODO [FBE] add parameter check

    // compute parameters into test parameter format
    lli_big_compute_params(p_param, &test_param);

    // use test command
    return (hci_le_create_big_test_cmd_handler(&test_param, opcode));
}
...
int hci_le_big_create_sync_cmd_handler(struct hci_le_big_create_sync_cmd const *p_param, uint16_t opcode)
{
    ...
    do
    {
        ...
        {
            if((p_param->bis_id[cursor] == 0) || (p_param->bis_id[cursor] >= BLE_BIS_MAX_NB))
            {
                status = CO_ERROR_INVALID_HCI_PARAM;
                break;
            }
        ...
        if(p_big_env == NULL)
        {
            ...
            for(cursor = 0 ; cursor < p_param->num_bis ; cursor++)
            {
                status = lli_bis_create(&p_bis_env, ROLE_SLAVE, p_big_env->group.hdl, p_param->bis_id[cursor]);
                ...
                // keep channel handle
                p_big_env->chan_hdl[cursor] = p_bis_env->chan.hdl;
            }
    ...
    if(status != CO_ERROR_NO_ERROR)
    {
        uint8_t max_chan = cursor;

        for(cursor = 0 ; cursor < max_chan; cursor++)
        {
            llm_activity_free_set(p_big_env->chan_hdl[cursor]);
            lli_chan_cleanup(p_big_env->chan_hdl[cursor]);
        }

        if(p_big_env!= NULL)
        {
            // Set activity in reserved state
            llm_activity_free_set(p_big_env->act_id);
            lli_big_cleanup(p_big_env->group.hdl);
        }
    }
...
int hci_le_terminate_big_cmd_handler(struct hci_le_terminate_big_cmd const *p_param, uint16_t opcode)
{
    ...
    do
    {
        ...
        p_big_env = lli_big_get(p_param->big_hdl);
        if((p_big_env == NULL) || !GETB(p_big_env->info, LLI_BIG_BROADCASTER))
        {
            status = CO_ERROR_UNKNOWN_ADVERTISING_ID;
            break;
        }
...
int hci_le_big_terminate_sync_cmd_handler(struct hci_le_big_terminate_sync_cmd const *p_param, uint16_t opcode)
{
    ...
    else if(status != CO_ERROR_NO_ERROR) // Only send the command complete event here if the driver is not stopping
    {
                              ...
                              ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    do
    {
        ...
        // Search the Slave BIS linked with sync handle
        ...
        // allocate start info structure from non-retention memory - will be free before having reason to go in deep sleep
        p_big_env->p_start = (struct lli_bi_start_params*) ke_malloc(sizeof(struct lli_bi_start_params), KE_MEM_NON_RETENTION);
        ...
        for(i = 0 ; i < p_big_env->nb_bis ; i++)
        {
            status = LLI_BIS_ENV_GET(p_big_env->chan_hdl[i], &p_bis_env);
            ASSERT_INFO(status == CO_ERROR_NO_ERROR, p_big_env->group.hdl, p_big_env->chan_hdl[i]);
        ...
        // BIS spacing and sub event interval are valid
        if(   (p_big_info->bis_spacing == 0)
              // BIS spacing and sub-event interval can be equals only for NSE = 1
           || ((p_big_info->nse > 1) && (p_big_info->bis_spacing == p_big_info->sub_interval)))
        {
            status = CO_ERROR_INVALID_LMP_PARAM;
            break;
        }
        else if(p_big_info->bis_spacing > p_big_info->sub_interval)
        {
        ...
        for(i = 0 ; i < p_big_env->nb_bis ; i++)
        {
            ...
            p_big_params->chan_params[i+1].chan_act_id = p_big_env->chan_hdl[i];
            p_big_params->chan_params[i+1].chan_idx    = p_bis_env->bis_id;
        }
        ...
        // *************************************************************************************************************
        // BIG not encrypted, BIG slave can be immediately started
        // *************************************************************************************************************
        else
        {
            // start the procedure
...
uint8_t lli_bis_stats_get(struct lli_chan_env *p_env, uint32_t* crc_error_packets, uint32_t* rx_unreceived_packets)
{
    ...
    do
    {
        ...
        status = lld_bis_stats_get(p_bis_env->chan.hdl, crc_error_packets, rx_unreceived_packets);
    } while (0);
